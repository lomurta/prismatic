...
void transfCPU_to_GPU(){
  ...
  /*  gpuErrchk(cudaMemcpy(d_QSURF->AXX, QSURF_.AXX, sizeof(double)*NS, cudaMemcpyHostToDevice));
    ...
    gpuErrchk(cudaMemcpyToSymbol(dg_QSURF_, &h_QSURF, sizeof(hd_QSURF)));
...
void transfGPU_to_CPU(){
    ...
    //QSURF
   // gpuErrchk(cudaMemcpyFromSymbol(h_QSURF, &dg_QSURF_, sizeof(hd_QSURF)));
...
void cpyQSURF(){
	...
	for (int i = 0; i < pilhaPart; i++){
		for (int j = 0; j < NSdiv; j++){
			h_QSURF.AXX[j][i] = QSURF_.AXX[j];
			h_QSURF.AXY[j][i] = QSURF_.AXY[j];
			h_QSURF.AXZ[j][i] = QSURF_.AXZ[j];
			h_QSURF.AYY[j][i] = QSURF_.AYY[j];
			h_QSURF.AYZ[j][i] = QSURF_.AYZ[j];
			h_QSURF.AZZ[j][i] = QSURF_.AZZ[j];
			h_QSURF.AX[j][i] = QSURF_.AX[j];
			h_QSURF.AY[j][i] = QSURF_.AY[j];
			h_QSURF.AZ[j][i] = QSURF_.AZ[j];
			h_QSURF.A0[j][i] = QSURF_.A0[j];
			h_QSURF.KPLANE[j][i] = QSURF_.KPLANE[j];
		}
		h_QSURF.NSURF[i] = *QSURF_.NSURF;
	}
}
