...
//L302:;
	...
	if (dg_TRACK_mod_.MAT[index] == 0) {
		...
		if (dg_TRACK_mod_.MAT[index] == 0) {//A particula não entrou no sistema
	...
	if (dg_TRACK_mod_.E[index] < dg_CSPGEO_.EABSB[dg_TRACK_mod_.IBODY[index] - 1][dg_TRACK_mod_.KPAR[index] - 1]) { //energia é muito baixa
		DEP = dg_TRACK_mod_.E[index] * dg_TRACK_mod_.WGHT[index];
		if ((dg_TRACK_mod_.KPAR[index] == 3) && (dg_TRACK_mod_.E[index] > 1.0e-6)) { //aniquilação de positrion
...
L103:;
	...
	if (NCROSS > 0) {
		...
		if (dg_TRACK_mod_.MAT[index] == 0) { //A partícula está fora do recinto.
	...
	if (dg_TRACK_mod_.E[index] < dg_CSPGEO_.EABSB[dg_TRACK_mod_.IBODY[index] - 1][dg_TRACK_mod_.KPAR[index] - 1]) { //A partícula foi absorvida.
		DE = DE + dg_TRACK_mod_.E[index];
		if ((dg_TRACK_mod_.KPAR[index] == 3) && (dg_TRACK_mod_.E[index] > 1.0e-6)) { //Aniquilação de positron
...
//	d_secpar2_(LEFT);
	//if (LEFT > 0) {
		...
		if (dg_TRACK_mod_.ILB[1 - 1][index] == 1) { //Fonte da particula primaria
		...
		if (dg_TRACK_mod_.E[index] > dg_CSPGEO_.EABSB[dg_TRACK_mod_.IBODY[index] - 1][dg_TRACK_mod_.KPAR[index] - 1]) {
				//Divisoes de raios X
				if (dg_TRACK_mod_.KPAR[index] == 2) {
					if (dg_TRACK_mod_.ILB[4 - 1][index] > 0) { //caracteristica de raio X
...
/*	if (index == 0){
	...
	for (int I = 1; I <= dg_QSURF_.NSURF[index]; I++) {
	...
	if (dg_TRACK_mod_.MAT[index] == 0) {
		//printf("index %d Step2 Particula no vacuo voa livrimento\n\n", index);
...
__device__ void d_fsurf2_(int& KS, double& A, double& B, double& C) {
	...
	*/
	...
	if (dg_QSURF_.KPLANE[KS - 1][index] == 0) {
		A = dg_TRACK_mod_.U[index] * (dg_QSURF_.AXX[KS - 1][index] * dg_TRACK_mod_.U[index] + dg_QSURF_.AXY[KS - 1][index] * dg_TRACK_mod_.V[index] + dg_QSURF_.AXZ[KS - 1][index] * dg_TRACK_mod_.W[index]) +
			dg_TRACK_mod_.V[index] * (dg_QSURF_.AYY[KS - 1][index] * dg_TRACK_mod_.V[index] + dg_QSURF_.AYZ[KS - 1][index] * dg_TRACK_mod_.W[index]) + dg_TRACK_mod_.W[index] * dg_QSURF_.AZZ[KS - 1][index] * dg_TRACK_mod_.W[index];
		XXX = dg_QSURF_.AXX[KS - 1][index] * dg_TRACK_mod_.X[index] + dg_QSURF_.AXY[KS - 1][index] * dg_TRACK_mod_.Y[index] + dg_QSURF_.AXZ[KS - 1][index] * dg_TRACK_mod_.Z[index] + dg_QSURF_.AX[KS - 1][index];
		YYY = dg_QSURF_.AYY[KS - 1][index] * dg_TRACK_mod_.Y[index] + dg_QSURF_.AYZ[KS - 1][index] * dg_TRACK_mod_.Z[index] + dg_QSURF_.AY[KS - 1][index];
		ZZZ = dg_QSURF_.AZZ[KS - 1][index] * dg_TRACK_mod_.Z[index] + dg_QSURF_.AZ[KS - 1][index];

		B = dg_TRACK_mod_.U[index] * (dg_QSURF_.AXX[KS - 1][index] * dg_TRACK_mod_.X[index] + XXX) + dg_TRACK_mod_.V[index] * (dg_QSURF_.AXY[KS - 1][index] * dg_TRACK_mod_.X[index] + dg_QSURF_.AYY[KS - 1][index] * dg_TRACK_mod_.Y[index] + YYY) +
			dg_TRACK_mod_.W[index] * (dg_QSURF_.AXZ[KS - 1][index] * dg_TRACK_mod_.X[index] + dg_QSURF_.AYZ[KS - 1][index] * dg_TRACK_mod_.Y[index] + dg_QSURF_.AZZ[KS - 1][index] * dg_TRACK_mod_.Z[index] + ZZZ);

		C = dg_TRACK_mod_.X[index] * XXX + dg_TRACK_mod_.Y[index] * YYY + dg_TRACK_mod_.Z[index] * ZZZ + dg_QSURF_.A0[KS - 1][index];
	}
	else {
		A = 0.0e0;
		B = dg_TRACK_mod_.U[index] * dg_QSURF_.AX[KS - 1][index] + dg_TRACK_mod_.V[index] * dg_QSURF_.AY[KS - 1][index] + dg_TRACK_mod_.W[index] * dg_QSURF_.AZ[KS - 1][index];
		C = dg_TRACK_mod_.X[index] * dg_QSURF_.AX[KS - 1][index] + dg_TRACK_mod_.Y[index] * dg_QSURF_.AY[KS - 1][index] + dg_TRACK_mod_.Z[index] * dg_QSURF_.AZ[KS - 1][index] + dg_QSURF_.A0[KS - 1][index];
	}
...
d100:
	...
	dg_TRACK_mod_.IBODY[index] = dg_QTREE_.NBODYS + 1;
	dg_TRACK_mod_.MAT[index] = 0;
	//printf("LCOATE setou para MAT 0\n\n");
...
__device__ void d_jump2_(double& DSMAX, double& DS) {
	...
	*/
	...
	if (dg_TRACK_mod_.KPAR[index] == 1) { //eletrons
		if (dg_CJUMP1_.MHINGE[index] == 1) {
	...
	else if (dg_TRACK_mod_.KPAR[index] == 3) { //Positrons
