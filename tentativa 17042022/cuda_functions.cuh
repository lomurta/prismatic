extern "C"{
	...
	__device__ void showers_ori(int size);
	__global__ void showers_ori2(int size);
	...
	__device__ void showers_sec(int size);
...
__device__ void showers_ori(int size){
	...
	if (index == 0)
		...
		for (int KB = 1; KB <= dg_PENGEOM_mod_.NBODY; KB++) {
			dg_CNT1_.DEBO[index][KB - 1] = 0.0e0; //Energias depositadas nos diversos corpos KB
		}
		...
		if (index == 0)
	printf("limpando a pilha secundaria\n");

		d_cleans2_(); //Limpa a pilha secundaria
	...
	/*	if (dg_CSOUR0_.KPARP == 0) { //não será implementado o tratamento para eletrons como particula primaria nesta versão do programas.
			...
			dg_CNTRL_.N[index] = dg_CNTRL_.N[index] + 1;

			if (dg_CNTRL_.N[index] > 2000000000)
				dg_CNTRL_.N[index] = dg_CNTRL_.N[index] - 2000000000;

			dg_TRACK_mod_[index].KPAR = dg_CSOUR0_.KPARP;
			dg_TRACK_mod_[index].WGHT = 1.0e0;

			//Posição inicial da particula
			if (dg_CSOUR3_.LEXSRC) {
				if (dg_CSOUR3_.LEXBD) {
					NTRIAL = 0;
				L301:;
					dg_TRACK_mod_[index].X = dg_CSOUR3_.SX0 + (d_rand2_(1.0e0) - 0.5e0) * dg_CSOUR3_.SSX;
					dg_TRACK_mod_[index].Y = dg_CSOUR3_.SY0 + (d_rand2_(2.0e0) - 0.5e0) * dg_CSOUR3_.SSY;
					dg_TRACK_mod_[index].Z = dg_CSOUR3_.SZ0 + (d_rand2_(3.0e0) - 0.5e0) * dg_CSOUR3_.SSZ;
					d_locate2_();
					NTRIAL = NTRIAL + 1;
					if (NTRIAL > 200) {
					...
					if (dg_CSOUR3_.IXSBOD[dg_TRACK_mod_[index].IBODY - 1] == 0)
						goto L301;
				}
				else {
					dg_TRACK_mod_[index].X = dg_CSOUR3_.SX0 + (d_rand2_(1.0e0) - 0.5e0) * dg_CSOUR3_.SSX;
				...
				dg_TRACK_mod_[index].X = dg_CSOUR3_.SX0;
				dg_TRACK_mod_[index].Y = dg_CSOUR3_.SY0;
				dg_TRACK_mod_[index].Z = dg_CSOUR3_.SZ0;
			}

			//Direção Inicial
			if (dg_CSOUR0_.LSCONE) {
				d_gcone2_(dg_TRACK_mod_[index].U, dg_TRACK_mod_[index].V, dg_TRACK_mod_[index].W); //Feixe Conico

			}
			else { //Feixe Retangular

				dg_TRACK_mod_[index].W = dg_CSOUR0_.CTHL + d_rand2_(4.0e0) * dg_CSOUR0_.DCTH;
				UV = sqrt(1.0e0 - dg_TRACK_mod_[index].W * dg_TRACK_mod_[index].W);
				PHI = dg_CSOUR0_.PHIL + d_rand2_(5.0e0) * dg_CSOUR0_.DPHI;
				dg_TRACK_mod_[index].U = UV * cos(PHI);
				dg_TRACK_mod_[index].V = UV * sin(PHI);
			}
			//Energia Inicial
			if (*CSOUR2_.LSPEC) {
				RN = d_rand2_(6.0e0) * *CNT2_.NSEB + 1;
				...
				dg_TRACK_mod_[index].E = CSOUR2_.ESRC[KEn - 1] + d_rand2_(7.0e0) * (CSOUR2_.ESRC[KEn + 1 - 1] - CSOUR2_.ESRC[KEn - 1]);
				CNT2_.SHIST[KEn - 1] = CNT2_.SHIST[KEn - 1] + 1.0e0;
			}
			else {
				dg_TRACK_mod_[index].E = *CSOUR1_.E0; //Fonte MonoEnergetica.
				CNT2_.SHIST[1 - 1] = CNT2_.SHIST[1 - 1] + 1.0e0;
			}

			dg_TRACK_mod_[index].ILB[1 - 1] = 1;  //Identifica partículas primárias.
			dg_TRACK_mod_[index].ILB[2 - 1] = 0;
			dg_TRACK_mod_[index].ILB[3 - 1] = 0;
			dg_TRACK_mod_[index].ILB[4 - 1] = 0;
			dg_TRACK_mod_[index].ILB[5 - 1] = 0;

			if (dg_TRACK_mod_[index].KPAR == 2) {
				if (dg_CSOUR0_.LGPOL) {
					dg_TRACK_mod_[index].IPOL = 1;  //Polarizacao de Fotons
					dg_TRACK_mod_[index].SP1 = *CSOUR1_.SP10;
					dg_TRACK_mod_[index].SP2 = *CSOUR1_.SP20;
					dg_TRACK_mod_[index].SP3 = *CSOUR1_.SP30;
				}
				else {
...
L302:;
	if (index == 0)
		...
		if (dg_TRACK_mod_[index].MAT == 0) {
			...
			if (dg_TRACK_mod_[index].MAT == 0) {//A particula não entrou no sistema
			...
			IDET = dg_PENGEOM_mod_.KDET[dg_TRACK_mod_[index].IBODY - 1];
			if (IDET != 0) {
				if ((dg_PENGEOM_mod_.KDET[IBODYL - 1] != IDET) && (dg_CNT4_.KKDI[dg_TRACK_mod_[index].KPAR - 1][IDET - 1] == 1)) {
					//Esse trecho faz gravação no arquivo Phase-Space que não será implementado nesta versao
					/*if (dg_CNT4_.IPSF[IDET-1] == 1){
						NSHJ=*CNTRL_.SHN - *dg_CNT4_.RLAST;
						wrpsf2_(dg_CNT4_.IPSFO,NSHJ,0);
						*dg_CNT4_.RWRITE=*dg_CNT4_.RWRITE+1.0e0;
						*dg_CNT4_.RLAST=*CNTRL_.SHN;*/

					d_simdet2_(dg_CNTRL_.N[index], IDET);

					if (dg_CNT4_.IDCUT[IDET - 1] == 0) {
						dg_CNT1_.DEBO[index][dg_TRACK_mod_[index].IBODY - 1] = dg_CNT1_.DEBO[index][dg_TRACK_mod_[index].IBODY - 1] + dg_TRACK_mod_[index].E * dg_TRACK_mod_[index].WGHT;
						IEXIT = 3;
						goto L104;
					}
				}
		...
		if (dg_TRACK_mod_[index].E < dg_CSPGEO_.EABSB[dg_TRACK_mod_[index].IBODY - 1][dg_TRACK_mod_[index].KPAR - 1]) { //energia é muito baixa
			DEP = dg_TRACK_mod_[index].E * dg_TRACK_mod_[index].WGHT;
			if ((dg_TRACK_mod_[index].KPAR == 3) && (dg_TRACK_mod_[index].E > 1.0e-6)) { //aniquilação de positrion
				d_panar2_(dg_CSPGEO_.EABSB[dg_TRACK_mod_[index].IBODY - 1][2 - 1]);
				DEP = DEP + TREV * dg_TRACK_mod_[index].WGHT;
			}
			dg_TRACK_mod_[index].E = 0.0e0;

			dg_CNT1_.DEBO[index][dg_TRACK_mod_[index].IBODY - 1] = dg_CNT1_.DEBO[index][dg_TRACK_mod_[index].IBODY - 1] + DEP;
			if (dg_CNT6_.LDOSEM) {
				d_sdose2_(DEP, dg_TRACK_mod_[index].X, dg_TRACK_mod_[index].Y, dg_TRACK_mod_[index].Z, dg_TRACK_mod_[index].MAT, dg_CNTRL_.N[index]);
			}
			IEXIT = 3;
			goto L104;
	...
	L102:;
	printf("\n\nL102 inicio da simulacao da particula\n\n");
	...
	L103:;
		...
		XL = dg_TRACK_mod_[index].X;
		YL = dg_TRACK_mod_[index].Y;
		ZL = dg_TRACK_mod_[index].Z;
		...
		d_jump2_(dg_CSPGEO_.DSMAX[dg_TRACK_mod_[index].IBODY - 1], DS);
		LINTF = false;
		...
		if (IDET != 0) {
			if (dg_CNT4_.IDCUT[IDET - 1] == 2) {
				if (dg_CNT4_.KKDI[dg_TRACK_mod_[index].KPAR - 1][IDET - 1] == 1) {
					if (dg_TRACK_mod_[index].KPAR == 2) {
						d_fimdet2_(dg_CNTRL_.N[index], IDET, DSEF);
					}
					else {
						DECSD = dg_PENELOPE_mod_.SSOFT[index] * DSEF;
						if (DECSD > 1.0e-12) { // A distribuição pode se estender
							d_fimdes2_(dg_CNTRL_.N[index], IDET, dg_PENELOPE_mod_.E0STEP[index], DECSD, DSEF); //abaixo do EABS.
						}
						else {
							d_fimdet2_(dg_CNTRL_.N[index], IDET, DSEF);
						}
			...
			if (dg_TRACK_mod_[index].KPAR != 2) {
				...
				if (dg_CJUMP1_[index].MHINGE == 0) {
					...
					if (dg_CNT6_.LDOSEM) {
						...
						XD = XL + dg_TRACK_mod_[index].U * DSEFR;
						YD = YL + dg_TRACK_mod_[index].V * DSEFR;
						ZD = ZL + dg_TRACK_mod_[index].W * DSEFR;
						d_sdose2_(DEP, XD, YD, ZD, MATL, dg_CNTRL_.N[index]);
					}
				}
				else {
					DEP = -dg_PENELOPE_mod_.SSOFT[index] * (DS - DSEF) * dg_TRACK_mod_[index].WGHT;
					if (dg_CNT6_.LDOSEM) {
						d_sdose2_(DEP, XL, YL, ZL, MATL, dg_CNTRL_.N[index]);
					}
				}
				dg_CNT1_.DEBO[index][IBODYL - 1] = dg_CNT1_.DEBO[index][IBODYL - 1] + DEP;
			}
		

			//Verifique se a partícula está fora do invólucro.
			if (dg_TRACK_mod_[index].MAT == 0) { //A partícula está fora do recinto.
		...
		if (dg_TRACK_mod_[index].E < dg_CSPGEO_.EABSB[dg_TRACK_mod_[index].IBODY - 1][dg_TRACK_mod_[index].KPAR - 1]) { //A partícula foi absorvida.
			DE = DE + dg_TRACK_mod_[index].E;
			if ((dg_TRACK_mod_[index].KPAR == 3) && (dg_TRACK_mod_[index].E > 1.0e-6)) { //Aniquilação de positron
		...
		DEP = DE * dg_TRACK_mod_[index].WGHT;
		dg_CNT1_.DEBO[index][dg_TRACK_mod_[index].IBODY - 1] = dg_CNT1_.DEBO[index][dg_TRACK_mod_[index].IBODY - 1] + DEP;
	...
	L104:;
		...
		if (dg_TRACK_mod_[index].ILB[1 - 1] == 1) {
			dg_CNT0_.DPRIM[index][IEXIT - 1] = dg_CNT0_.DPRIM[index][IEXIT - 1] + dg_TRACK_mod_[index].WGHT;
			/*if (dg_CSOUR0_.LPSF){  não sera implementado redução de variancia como divisão de particulas ou roleta russa
				if (NSPL1 > 1)
					dg_CNT0_.DPRIM[index][IEXIT-1]=dg_CNT0_.DPRIM[index][IEXIT-1]+dg_TRACK_mod_[index].WGHT*(NSPL1-1);
			}
			*/

			if (IEXIT < 3) {
				dg_CNT0_.DAVW[index][IEXIT - 1] = dg_CNT0_.DAVW[index][IEXIT - 1] + dg_TRACK_mod_[index].W * dg_TRACK_mod_[index].WGHT;
				dg_CNT0_.DAVA[index][IEXIT - 1] = dg_CNT0_.DAVA[index][IEXIT - 1] + acos(dg_TRACK_mod_[index].W) * dg_TRACK_mod_[index].WGHT;
				dg_CNT0_.DAVE[index][IEXIT - 1] = dg_CNT0_.DAVE[index][IEXIT - 1] + dg_TRACK_mod_[index].E * dg_TRACK_mod_[index].WGHT;
			}

		}
		else {
			dg_CNT0_.DSEC[index][IEXIT - 1][dg_TRACK_mod_[index].KPAR - 1] = dg_CNT0_.DSEC[index][IEXIT - 1][dg_TRACK_mod_[index].KPAR - 1] + dg_TRACK_mod_[index].WGHT;
		}

		if (IEXIT < 3) {
			d_tenang2_(IEXIT, dg_CNTRL_.N[index]);  //Energia e distribuições angulares.
		}
	...
	L202:;
		...
		if (LEFT > 0) {
			...
			if (dg_TRACK_mod_[index].E > dg_CSPGEO_.EABSB[dg_TRACK_mod_[index].IBODY - 1][dg_TRACK_mod_[index].KPAR - 1]) {
				...
				if (dg_CNT6_.LDOSEM) {
					double wDEP = -DEP;
					d_sdose2_(wDEP, dg_TRACK_mod_[index].X, dg_TRACK_mod_[index].Y, dg_TRACK_mod_[index].Z, dg_TRACK_mod_[index].MAT, dg_CNTRL_.N[index]);
				}
			}
			else {
				goto L202;
		...
		/*if (dg_CSOUR0_.LPSF){ Não sera implementado o Phase-Space
			if (ISEC == 1)
				goto L201;
	...
	L202:;
		...
		if (LEFT > 0)
		{
			...
			if (dg_TRACK_mod_[index].E > dg_CSPGEO_.EABSB[dg_TRACK_mod_[index].IBODY - 1][dg_TRACK_mod_[index].KPAR - 1])
			{
				...
				DEP = dg_TRACK_mod_[index].E * dg_TRACK_mod_[index].WGHT;
				// dg_CNT1_.DEBO[index][dg_TRACK_mod_[index].IBODY - 1] = dg_CNT1_.DEBO[index][dg_TRACK_mod_[index].IBODY - 1] - DEP;
				dg_CNT1_.DEBO[dg_TRACK_mod_[index].N][dg_TRACK_mod_[index].IBODY - 1] = dg_CNT1_.DEBO[dg_TRACK_mod_[index].N][dg_TRACK_mod_[index].IBODY - 1] - DEP;
...
/*		 if ((index == 0) || (index == 1) || (index == 4095) || (index == 4096) || (index == 256)){
	...
	/*	if ((index == 0) || (index == 1)){
			printf("SHOWER SEC DEBO[%d]: %f\n", index, dg_CNT1_.DEBO[index][dg_TRACK_mod_[index].IBODY - 1]);
		}*/
...
/*	if (index == 0){
	...
	for (int I = 1; I <= dg_QSURF_.NSURF; I++) {
		dg_QTREE_.KSP[index][I - 1] = 0; //Ponteiros laterais das superfícies avaliadas.
	}
	...
	if (dg_TRACK_mod_[index].IBODY > dg_QTREE_.NBODYS) {
		...
		for (int KI = NSCT; KI >= 1; KI--) {
			...
			if (dg_QTREE_.KSP[index][dg_PENGEOM_mod_.KSLAST[index] - 1] == 1)
				dg_QTREE_.KSP[index][dg_PENGEOM_mod_.KSLAST[index] - 1] = 2;
			else
				dg_QTREE_.KSP[index][dg_PENGEOM_mod_.KSLAST[index] - 1] = 1;
			...
			for (int KSS = 1; KSS <= NST; KSS++) {
				...
				if ((KF < 3) && (dg_QTREE_.KSP[index][KS1 - 1] != KF))
					goto L101;
			}
...
L102:;
	...
	if (IERR != 0) {
		...
		if (dg_PENGEOM_mod_.LVERB) {
			...
			NERR = NERR + 1;
			if ((dg_QTREE_.NWARN[index] < 100) && (dg_TRACK_mod_[index].MAT != 0)) {
				...
				for (int KSS = 1; KSS <= dg_QTREE_.KSURF[NXG - 1][KB1 - 1]; KSS++) {
					...
					L103:;
						d_fsurf2_(KS, A, B, C);
						if (KFLO == dg_QTREE_.KSP[index][KS - 1]) {
	...
	if ((dg_TRACK_mod_[index].MAT != 0) && (DSRES < d_S[NSC - 1])) {
		if (dg_TRACK_mod_[index].MAT == MAT0)
			DSEF = DSEF + DSRES;
...
__device__ void d_stepsi2_(int& KB, double *d_S, int *d_IS,  int& NSC) {
	...
	for (int KSS = 1; KSS <= dg_QTREE_.KSURF[NXG - 1][KB - 1]; KSS++) {
		...
		if (ABSA < 1.0e-36) {
			if (ABSB > 0.0e0) {
				if (C < -FUZZL) {
					dg_QTREE_.KSP[index][KS - 1] = 1;
				}
				else if (C > FUZZL) {
					dg_QTREE_.KSP[index][KS - 1] = 2;
				}
				else {
					if (B < 0.0e0) {
						dg_QTREE_.KSP[index][KS - 1] = 1;
		...
		if (NLBOD > 0) {
			for (int KBB = 1; KBB <= NLBOD; KBB++) {
				...
				for (int KSS = 1; KSS <= dg_QTREE_.KSURF[NXG - 1][KBS - 1]; KSS++) {
					...
					if ((KF < 3) && (dg_QTREE_.KSP[index][KS - 1] != KF))
						goto L100;
				}
...
d100:
	for (int KSS = 1; KSS <= dg_QTREE_.KSURF[NXG - 1][KB0 - 1]; KSS++) {
		...
		if ((dg_QTREE_.KSP[index][KS - 1] != 0) || (dg_QTREE_.KFLAG[KSS - 1][KB0 - 1] > 4)) {
			goto d101;
		}
		...
		if (C < (-FUZZ)) {
			...
			if (B < 0.0e0)
				dg_QTREE_.KSP[index][KS - 1] = 1; //particula movendo-se para dentro
			else
				dg_QTREE_.KSP[index][KS - 1] = 2; // particula movendo-se para fora
		}
	d101:;
	}
...
__device__ void d_secpar2_(int& LEFT) {
	...
	*/
	int index = blockDim.x * blockIdx.x + threadIdx.x;
	if (dg_SECST_[index].NSEC > 0) {
		LEFT = dg_SECST_[index].NSEC;
		if (index == 0)
			printf("SECST NSEC %d\n", dg_SECST_[index].NSEC );
		dg_SECST_[index].NSEC = dg_SECST_[index].NSEC - 1;
		dg_TRACK_mod_[index].E = dg_SECST_[index].ES[dg_SECST_[index].NSEC];
		dg_TRACK_mod_[index].X = dg_SECST_[index].XS[dg_SECST_[index].NSEC];
		dg_TRACK_mod_[index].Y = dg_SECST_[index].YS[dg_SECST_[index].NSEC];
		dg_TRACK_mod_[index].Z = dg_SECST_[index].ZS[dg_SECST_[index].NSEC];
		dg_TRACK_mod_[index].U = dg_SECST_[index].US[dg_SECST_[index].NSEC];
		dg_TRACK_mod_[index].V = dg_SECST_[index].VS[dg_SECST_[index].NSEC];
		dg_TRACK_mod_[index].W = dg_SECST_[index].WS[dg_SECST_[index].NSEC];
		dg_TRACK_mod_[index].WGHT = dg_SECST_[index].WGHTS[dg_SECST_[index].NSEC];
		dg_TRACK_mod_[index].KPAR = dg_SECST_[index].KS[dg_SECST_[index].NSEC];
		dg_TRACK_mod_[index].IBODY = dg_SECST_[index].IBODYS[dg_SECST_[index].NSEC];
		dg_TRACK_mod_[index].MAT = dg_SECST_[index].MS[dg_SECST_[index].NSEC];
		dg_TRACK_mod_[index].IPOL = dg_SECST_[index].IPOLS[dg_SECST_[index].NSEC];
		dg_TRACK_mod_[index].SP1 = dg_SECST_[index].SP1S[dg_SECST_[index].NSEC];
		dg_TRACK_mod_[index].SP2 = dg_SECST_[index].SP2S[dg_SECST_[index].NSEC];
		dg_TRACK_mod_[index].SP3 = dg_SECST_[index].SP3S[dg_SECST_[index].NSEC];
		dg_TRACK_mod_[index].PAGE = dg_SECST_[index].PAGES[dg_SECST_[index].NSEC];
		for (int I = 1; I <= 5; I++) {
			dg_TRACK_mod_[index].ILB[I - 1] = dg_SECST_[index].ILBS[dg_SECST_[index].NSEC][I - 1];
		}
	}
	else {
		LEFT = 0;
...
__device__ void d_gpha2_(double& ES, int& IZZ, int& ISH) {
	...
	*/
	...
	if (PTOT * dg_COMPOS_.VMOL[dg_TRACK_mod_[index].MAT - 1] > dg_CGIMFP_.SGPH[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) {
		printf("WARNING: SGPH is less than the actual mac.\n");
	}

	//Faça uma amostra do elemento ativo.
	TST = d_rand2_(1.0e0) * dg_CGIMFP_.SGPH[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] / dg_COMPOS_.VMOL[dg_TRACK_mod_[index].MAT - 1];
	for (int IEL = 1; IEL <= dg_COMPOS_.NELEM[dg_TRACK_mod_[index].MAT - 1]; IEL++) {
		if (ACP[IEL - 1] > TST) {
			IELAC = IEL;
			IZZ = dg_COMPOS_.IZ[IEL - 1][dg_TRACK_mod_[index].MAT - 1];
...
L3:;
	//Eletron
	EE = EPS * dg_TRACK_mod_[index].E - REV;
	CDTE = 2.0e0 * d_rand2_(7.0e0) - 1.0e0;
	A1 = EE + REV;
	A2 = sqrt(EE * (EE + TREV));
	CDTE = (CDTE * A1 + A2) / (A1 + CDTE * A2);
	//Positron
	EP = (1.0e0 - EPS) * dg_TRACK_mod_[index].E - REV;
	...
	TRIPL = dg_CGPP01_.TRIP[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CGPP01_.TRIP[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CGPP01_.TRIP[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index];
	IZZ = 0;
	ISH = 30;
	if (TRIPL < 1.0e-5)
		return;
...
__device__ void d_stores2new_(double& EI, double& XI, double& YI, double& ZI, double& UI, double& VI, double& WI, double& WGHTI, int& KPARI, int* ILBI, int& IPOLI) {
	...
	*/
	...
	if (KPARI == 1){
		if (dg_nTRACKS_.nSECTRACK_E == pilhaSec*10){
			for (int I = 0; I < pilhaPart*10; I++) {
...
__device__ void d_stores2_(double& EI, double& XI, double& YI, double& ZI, double& UI, double& VI, double& WI, double& WGHTI, int& KPARI, int* ILBI, int& IPOLI) {
	...
	*/
	...
	if (dg_SECST_[index].NSEC < NMS) {
		
		IS = dg_SECST_[index].NSEC;
		dg_SECST_[index].NSEC = dg_SECST_[index].NSEC + 1;
	}
	else {
		...
		dg_SECST_[index].NSEC = NMS;
		EME = 1.0e35;
		EMG = 1.0e35;
		IE = 0;
		IG = 0;
		for (int I = 0; I < NMS; I++) {
			if (dg_SECST_[index].KS[I] == 1) {
				if (dg_SECST_[index].ES[I] < EME) {
					EME = dg_SECST_[index].ES[I];
					IE = I;
				}
			}
			else if (dg_SECST_[index].KS[I] == 2) {
				if (dg_SECST_[index].ES[I] < EMG) {
					EMG = dg_SECST_[index].ES[I];
					IG = I;
				}
			}
		}
	...
	dg_SECST_[index].ES[IS] = EI;
	dg_SECST_[index].XS[IS] = XI;
	dg_SECST_[index].YS[IS] = YI;
	dg_SECST_[index].ZS[IS] = ZI;
	dg_SECST_[index].US[IS] = UI;
	dg_SECST_[index].VS[IS] = VI;
	dg_SECST_[index].WS[IS] = WI;
	dg_SECST_[index].WGHTS[IS] = WGHTI;
	dg_SECST_[index].KS[IS] = KPARI;
	dg_SECST_[index].IBODYS[IS] = dg_TRACK_mod_[index].IBODY;
	dg_SECST_[index].MS[IS] = dg_TRACK_mod_[index].MAT;
	dg_SECST_[index].ILBS[IS][1 - 1] = ILBI[1 - 1];
	dg_SECST_[index].ILBS[IS][2 - 1] = ILBI[2 - 1];
	dg_SECST_[index].ILBS[IS][3 - 1] = ILBI[3 - 1];
	dg_SECST_[index].ILBS[IS][4 - 1] = ILBI[4 - 1];
	dg_SECST_[index].ILBS[IS][5 - 1] = ILBI[5 - 1];

	if (IPOLI == 1) {
		dg_SECST_[index].SP1S[IS] = dg_TRACK_mod_[index].SP1;
		dg_SECST_[index].SP2S[IS] = dg_TRACK_mod_[index].SP2;
		dg_SECST_[index].SP3S[IS] = dg_TRACK_mod_[index].SP3;
		dg_SECST_[index].IPOLS[IS] = IPOLI;
	}
	else {
		dg_SECST_[index].SP1S[IS] = 0.e0;
		dg_SECST_[index].SP2S[IS] = 0.e0;
		dg_SECST_[index].SP3S[IS] = 0.e0;
		dg_SECST_[index].IPOLS[IS] = 0;
	}

	dg_SECST_[index].PAGES[IS] = dg_TRACK_mod_[index].PAGE;
...
L1000:;
	...
	if (dg_CJUMP1_[index].KSOFTI == 1) {
		...
		dg_PENELOPE_mod_.E0STEP[index] = dg_PENELOPE_mod_.E0STEP[index] - dg_PENELOPE_mod_.SSOFT[index] * (dg_CJUMP0_[index].DST - dg_CJUMP0_[index].DSR);
	...
	//Deflexão Angular
	if (dg_CEIMFP_.T1E[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] > -78.3e0) {
		dg_CJUMP0_[index].T1 = exp(dg_CEIMFP_.T1E[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CEIMFP_.T1E[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CEIMFP_.T1E[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
		dg_CJUMP0_[index].T2 = exp(dg_CEIMFP_.T2E[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CEIMFP_.T2E[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CEIMFP_.T2E[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
	}
	else {
		dg_CJUMP0_[index].T1 = 0.0e0;
		dg_CJUMP0_[index].T2 = 0.0e0;
	}
	if (dg_CJUMP0_[index].T1 < 1.0e-20)
		return;
	//1º e 2º momentos da distribuição angular.
	EMU1 = 0.5e0 * (1.0e0 - exp(-dg_CJUMP0_[index].DST * dg_CJUMP0_[index].T1));
	EMU2 = EMU1 - (1.0e0 - exp(-dg_CJUMP0_[index].DST * dg_CJUMP0_[index].T2)) / 6.0e0;
...
L1100:;
	...
	STNOW = dg_CJUMP0_[index].P[2 - 1] + dg_CJUMP0_[index].P[3 - 1] + dg_CJUMP0_[index].P[4 - 1] + dg_CJUMP0_[index].P[5 - 1] + dg_CJUMP0_[index].P[8 - 1];
	STS = fmax(STNOW, dg_CJUMP0_[index].ST) * d_rand2_(4.0e0);
	SS = dg_CJUMP0_[index].P[2 - 1];
	if (SS > STS)
		goto L1200;

	SS = SS + dg_CJUMP0_[index].P[3 - 1];
	if (SS > STS)
		goto L1300;

	SS = SS + dg_CJUMP0_[index].P[4 - 1];
	if (SS > STS)
		goto L1400;

	SS = SS + dg_CJUMP0_[index].P[5 - 1];
	if (SS > STS)
		goto L1500;

	SS = SS + dg_CJUMP0_[index].P[8 - 1];
	if (SS > STS)
		goto L1800;
...
L1200:;
	...
	if (dg_TRACK_mod_[index].E >= dg_CELSEP_.EELMAX[dg_TRACK_mod_[index].MAT - 1]) {
		TRNDC = dg_CEIMFP_.RNDCE[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CEIMFP_.RNDCE[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CEIMFP_.RNDCE[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index];
		TA = exp(dg_CEIMFP_.AE[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CEIMFP_.AE[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CEIMFP_.AE[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
		TB = dg_CEIMFP_.BE[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CEIMFP_.BE[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CEIMFP_.BE[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index];
		d_eela2_(TA, TB, TRNDC, RMU);
	}
	else {
		//Implementacao do modelo alternativo utilzando  ELSEPA database
		TRNDC = dg_CELSEP_.RNDCED[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CELSEP_.RNDCED[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CELSEP_.RNDCED[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index];
		d_eeld2_(TRNDC, RMU);
	}
...
L1300:;
	ICOL = 3;
	DELTA = dg_CEIMFP_.DEL[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CEIMFP_.DEL[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CEIMFP_.DEL[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index];
...
L2000:;
	...
	STS = dg_CJUMP0_[index].ST * d_rand2_(1.0e0);
	SS = dg_CJUMP0_[index].P[1 - 1];
	if (SS > STS)
		goto L2100;

	SS = SS + dg_CJUMP0_[index].P[2 - 1];
	if (SS > STS)
		goto L2200;
...
L2200:;
	...
	if (EP > dg_PENELOPE_mod_.EABS[dg_TRACK_mod_[index].MAT - 1][2 - 1]) {
		if (dg_TRACK_mod_[index].IPOL == 1) {
			ECDT = dg_TRACK_mod_[index].E * RREV * (1.0e0 - CDT);
...
L3000:;
	...
	//Deflexão Angular
	if (dg_CPIMFP_.T1P[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] > -78.3e0) {
		dg_CJUMP0_[index].T1 = exp(dg_CPIMFP_.T1P[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CPIMFP_.T1P[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CPIMFP_.T1P[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
		dg_CJUMP0_[index].T2 = exp(dg_CPIMFP_.T2P[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CPIMFP_.T2P[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CPIMFP_.T2P[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
	}
	else {
		dg_CJUMP0_[index].T1 = 0.0e0;
		dg_CJUMP0_[index].T2 = 0.0e0;
...
L3100:;
	...
	//Amostragem aleatória do tipo de interação.
	STNOW = dg_CJUMP0_[index].P[2 - 1] + dg_CJUMP0_[index].P[3 - 1] + dg_CJUMP0_[index].P[4 - 1] + dg_CJUMP0_[index].P[5 - 1] + dg_CJUMP0_[index].P[6 - 1] + dg_CJUMP0_[index].P[8 - 1];
	STS = fmax(STNOW, dg_CJUMP0_[index].ST) * d_rand2_(4.0e0);
	SS = dg_CJUMP0_[index].P[2 - 1];
	if (SS > STS)
		goto L3200;
...
L3200:;
	...
	if (dg_TRACK_mod_[index].E >= dg_CELSEP_.PELMAX[dg_TRACK_mod_[index].MAT - 1]) {
		TRNDC = dg_CPIMFP_.RNDCP[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CPIMFP_.RNDCP[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CPIMFP_.RNDCP[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index];
		TA = exp(dg_CPIMFP_.AP[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CPIMFP_.AP[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CPIMFP_.AP[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
		TB = dg_CPIMFP_.BP[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CPIMFP_.BP[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CPIMFP_.BP[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index];
		d_eela2_(TA, TB, TRNDC, RMU);
	}
	else {
		//Implementacao do modelo alternativo utilzando  ELSEPA database
		TRNDC = dg_CELSEP_.RNDCPD[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CELSEP_.RNDCPD[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CELSEP_.RNDCPD[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index];
		d_peld2_(TRNDC, RMU);
	}
...
L3400:;
	...
	if (dg_TRACK_mod_[index].E < dg_PENELOPE_mod_.EABS[dg_TRACK_mod_[index].MAT - 1][3 - 1]) {
		d_panar2_(dg_PENELOPE_mod_.EABS[dg_TRACK_mod_[index].MAT - 1][2 - 1]);//Aniquilação em repouso.
		DE = dg_TRACK_mod_[index].E + DE + TREV;
		dg_TRACK_mod_[index].E = 0.0e0;
	}
	return;
...
__device__ void d_panar2_(double& ECUT) {
	...
	int WKPARP = 2;
	d_stores2_(REV, dg_TRACK_mod_[index].X, dg_TRACK_mod_[index].Y, dg_TRACK_mod_[index].Z, US, VS, WS, dg_TRACK_mod_[index].WGHT, WKPARP, ILBA, d_wIPOLI);
	US = -US; VS = -VS; WS = -WS;
	d_stores2_(REV, dg_TRACK_mod_[index].X, dg_TRACK_mod_[index].Y, dg_TRACK_mod_[index].Z, US, VS, WS, dg_TRACK_mod_[index].WGHT, WKPARP, ILBA, d_wIPOLI);
}
...
__device__ void d_pimfp2_(int& IEND) {
	...
	*/
	...
	int index = blockDim.x * blockIdx.x + threadIdx.x;
	dg_CJUMP0_[index].P[2 - 1] = exp(dg_CPIMFP_.SPHEL[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CPIMFP_.SPHEL[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CPIMFP_.SPHEL[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
	dg_CJUMP0_[index].P[3 - 1] = exp(dg_CPIMFP_.SPHIN[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CPIMFP_.SPHIN[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CPIMFP_.SPHIN[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
	dg_CJUMP0_[index].P[4 - 1] = exp(dg_CPIMFP_.SPHBR[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CPIMFP_.SPHBR[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CPIMFP_.SPHBR[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
	dg_CJUMP0_[index].P[5 - 1] = exp(dg_CPIMFP_.SPISI[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CPIMFP_.SPISI[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CPIMFP_.SPISI[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
	dg_CJUMP0_[index].P[6 - 1] = exp(dg_CPIMFP_.SPAN[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CPIMFP_.SPAN[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CPIMFP_.SPAN[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
	dg_CJUMP0_[index].P[8 - 1] = 0.0e0;

	if (IEND == 1) {
		return;
	}

	if (dg_CPIMFP_.W1P[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] > -78.3e0) {
		dg_CJUMP0_[index].W1 = exp(dg_CPIMFP_.W1P[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CPIMFP_.W1P[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CPIMFP_.W1P[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
		dg_CJUMP0_[index].W2 = exp(dg_CPIMFP_.W2P[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CPIMFP_.W2P[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CPIMFP_.W2P[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
	}
	else {
		dg_CJUMP0_[index].W1 = 0.0e0;
		dg_CJUMP0_[index].W2 = 0.0e0;
	}
...
__device__ void d_eimfp2_(int& IEND) {
	...
	*/
	int index = blockDim.x * blockIdx.x + threadIdx.x;
	dg_CJUMP0_[index].P[2 - 1] = exp(dg_CEIMFP_.SEHEL[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CEIMFP_.SEHEL[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CEIMFP_.SEHEL[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
	dg_CJUMP0_[index].P[3 - 1] = exp(dg_CEIMFP_.SEHIN[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CEIMFP_.SEHIN[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CEIMFP_.SEHIN[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
	dg_CJUMP0_[index].P[4 - 1] = exp(dg_CEIMFP_.SEHBR[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CEIMFP_.SEHBR[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CEIMFP_.SEHBR[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
	dg_CJUMP0_[index].P[5 - 1] = exp(dg_CEIMFP_.SEISI[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CEIMFP_.SEISI[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CEIMFP_.SEISI[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
	dg_CJUMP0_[index].P[8 - 1] = 0.0e0;
	...
	if (dg_CEIMFP_.W1E[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] > -78.3e0) {
		dg_CJUMP0_[index].W1 = exp(dg_CEIMFP_.W1E[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CEIMFP_.W1E[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CEIMFP_.W1E[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
		dg_CJUMP0_[index].W2 = exp(dg_CEIMFP_.W2E[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CEIMFP_.W2E[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CEIMFP_.W2E[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
	}
	else {
		dg_CJUMP0_[index].W1 = 0.0e0;
		dg_CJUMP0_[index].W2 = 0.0e0;
...
__device__ void d_gimfp2_() {
	...
	*/
	...
	int index = blockDim.x * blockIdx.x + threadIdx.x;
	dg_CJUMP0_[index].P[1 - 1] = dg_CGIMFP_.SGRA[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1];
	dg_CJUMP0_[index].P[2 - 1] = exp(dg_CGIMFP_.SGCO[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CGIMFP_.SGCO[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CGIMFP_.SGCO[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
	dg_CJUMP0_[index].P[3 - 1] = dg_CGIMFP_.SGPH[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1];

	if (dg_TRACK_mod_[index].E < 1.023e6) {
		dg_CJUMP0_[index].P[4 - 1] = 0.0e0;
	}
	else {
		dg_CJUMP0_[index].P[4 - 1] = exp(dg_CGIMFP_.SGPP[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] + (dg_CGIMFP_.SGPP[dg_CEGRID_.KE[index] + 1 - 1][dg_TRACK_mod_[index].MAT - 1] - dg_CGIMFP_.SGPP[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1]) * dg_CEGRID_.XEK[index]);
	}
	dg_CJUMP0_[index].P[8 - 1] = 0.0e0;
}
...
__device__ void d_jump2_(double& DSMAX, double& DS) {
	...
	*/
	...
	if (dg_TRACK_mod_[index].KPAR == 1) { //eletrons
		...
		dg_CJUMP0_[index].ST = dg_CJUMP0_[index].P[2 - 1] + dg_CJUMP0_[index].P[3 - 1] + dg_CJUMP0_[index].P[4 - 1] + dg_CJUMP0_[index].P[5 - 1] + dg_CJUMP0_[index].P[8 - 1];
		DSMAXP = DSMAX;
		...
		*/
		...
		if (dg_CJUMP0_[index].W1 > 1.0e-20) {
			...
			*/
			DSMC = 4.0e0 / dg_CJUMP0_[index].ST;
			if (DSMAXP > DSMC) {
				DSMAXP = DSMC;
			}
			...
			EDE0 = dg_CJUMP0_[index].W1 * DSMAXP;
			VDE0 = dg_CJUMP0_[index].W2 * DSMAXP;
			FSEDE = fmax(1.0e0 - dg_CEIMFP_.DW1EL[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] * EDE0, 0.75e0);
			FSVDE = fmax(1.0e0 - dg_CEIMFP_.DW2EL[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] * EDE0, 0.75e0);
			EDEM = EDE0 * FSEDE;
			VDEM = VDE0 * FSVDE;
			W21 = VDEM / EDEM;
			...
			dg_CJUMP0_[index].ST = fmax(dg_CJUMP0_[index].ST, STLWR);
		}
		else {
		...
		*/
		...
		dg_CJUMP0_[index].DST = -log(d_rand2_(2.0e0)) / dg_CJUMP0_[index].ST;
		if (dg_CJUMP0_[index].DST < DSMAXP) {
			dg_CJUMP1_[index].KDELTA = 0;
		}
		else {
			dg_CJUMP0_[index].DST = DSMAXP;
			dg_CJUMP1_[index].KDELTA = 1;
		}

		if (dg_CJUMP1_[index].KSOFTE + dg_CJUMP1_[index].KSOFTI == 0) {
			dg_CJUMP1_[index].MHINGE = 1;
			DS = dg_CJUMP0_[index].DST;
		}
		else {
			DS = dg_CJUMP0_[index].DST * d_rand2_(3.0e0);
			dg_CJUMP0_[index].DSR = dg_CJUMP0_[index].DST - DS;
			if (dg_CJUMP1_[index].KSOFTI == 1) {
				if (dg_CJUMP0_[index].DST < 1.0e-8) {
					dg_PENELOPE_mod_.SSOFT[index] = dg_CJUMP0_[index].W1;
					dg_PENELOPE_mod_.DESOFT[index] = dg_PENELOPE_mod_.SSOFT[index] * dg_CJUMP0_[index].DST;
				}
				else {
					EDE0 = dg_CJUMP0_[index].W1 * dg_CJUMP0_[index].DST;
					VDE0 = dg_CJUMP0_[index].W2 * dg_CJUMP0_[index].DST;
					FSEDE = fmax(1.0e0 - dg_CEIMFP_.DW1EL[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] * EDE0, 0.75e0);
					FSVDE = fmax(1.0e0 - dg_CEIMFP_.DW2EL[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] * EDE0, 0.75e0);
					EDE = EDE0 * FSEDE;
					VDE = VDE0 * FSVDE;
					...
					dg_PENELOPE_mod_.SSOFT[index] = dg_PENELOPE_mod_.DESOFT[index] / dg_CJUMP0_[index].DST;
				}
			}
		}
		return;
		...
		//Caminho livre médio rígido inverso (probabilidade de interação por unidade comprimento do caminho).
		dg_CJUMP0_[index].ST = dg_CJUMP0_[index].P[2 - 1] + dg_CJUMP0_[index].P[3 - 1] + dg_CJUMP0_[index].P[4 - 1] + dg_CJUMP0_[index].P[5 - 1] + dg_CJUMP0_[index].P[6 - 1] + dg_CJUMP0_[index].P[8 - 1];
		DSMAXP = DSMAX;
		...
		*/
		...
		if (dg_CJUMP0_[index].W1 > 1.0e-20) {
			...
			*/
			...
			EDE0 = dg_CJUMP0_[index].W1 * DSMAXP;
			VDE0 = dg_CJUMP0_[index].W2 * DSMAXP;
			FSEDE = fmax(1.0e0 - dg_CPIMFP_.DW1PL[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] * EDE0, 0.75e0);
			FSVDE = fmax(1.0e0 - dg_CPIMFP_.DW2PL[dg_CEGRID_.KE[index] - 1][dg_TRACK_mod_[index].MAT - 1] * EDE0, 0.75e0);
			EDEM = EDE0 * FSEDE;
			VDEM = VDE0 * FSVDE;
			W21 = VDEM / EDEM;
		...
		*/
		...
		if (dg_TRACK_mod_[index].E < dg_CJUMP1_[index].ELAST1) {
			...
			d_gimfp2_();
			dg_CJUMP1_[index].ELAST1 = dg_TRACK_mod_[index].E;
			dg_CJUMP0_[index].ST = dg_CJUMP0_[index].P[1 - 1] + dg_CJUMP0_[index].P[2 - 1] + dg_CJUMP0_[index].P[3 - 1] + dg_CJUMP0_[index].P[4 - 1] + dg_CJUMP0_[index].P[8 - 1];
		}

		DS = -log(d_rand2_(1.0e0)) / dg_CJUMP0_[index].ST;
	}
