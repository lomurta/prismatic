...
C  ************  Initialise parameters.
	...
	CALL GEOMIN2(PARINP,NPINP,NMATG,NBOD,IRD,IWR)
	return;
...
C  ****  Expanded quadric.
 102  CONTINUE
	  ...
	  WRITE(*, *)
	  WRITE(*,'(''FORTRAN ANTES KS:'', I2)') KS
	  WRITE(*,'(''QXX2:'',D20.8)') QXX
	  WRITE(*,'(''QXY:'',D20.8)') QXY
	  WRITE(*,'(''QXZ:'',D20.8)') QXZ
	  WRITE(*,'(''QYY:'',D20.8)') QYY
	  WRITE(*,'(''QYZ:'',D20.8)') QYZ
...
C  ****  Rotated and shifted quadric.
     ...
     1   ABS(MAX(QXX,QXY,QXZ,QYY,QYZ,QZZ))).LT.1.0D-30) KPLANE(KS)=1
	  ...
	  WRITE(*, *)
	  WRITE(*,'(''FORTRAN DEPOIS KS:'', I2)') KS
	   WRITE(*, '(''KPLANE:'', I2)') KPLANE(KS)
	  WRITE(*,'(''QXX:'',D20.8)') QXX
	  WRITE(*,'(''QXY:'',D20.8)') QXY
	  WRITE(*,'(''QXZ:'',D20.8)') QXZ
	  WRITE(*,'(''QYY:'',D20.8)') QYY
...
C
      ...
      IF(MLESS.EQ.1) THEN
        IF(KBOMO(KBENC).EQ.1) THEN
			WRITE(*, '(''PULOU 602'')')
			GOTO 602  ! There is a root module.
		ENDIF	
      ENDIF
C
...
C	  CALL transfQTREE(NBODYS,KMOTH,KDGHT,KSURF, KFLAG,KSP, NWARN)

C	  CALL transfQSURF(AXX,AXY,AXZ,AYY,AYZ,AZZ,
C     1    AX,AY,AZ,A0,NSURF,KPLANE)
	 
	

C	 CALL transfTRACK_mod(E ,X ,Y,Z,U,V,W,WGHT,
C     1    SP1,SP2,SP3,PAGE,KPAR,IBODY,MAT,ILB,IPOL)
	 
C	 CALL transfPENELOPE_mod(EABS,C1,C2,WCC,WCR,DEN,
C     1    RDEN,E0STEP,DESOFT,SSOFT,NMS,NEGP, NMAT)
	 
C	 CALL transfPENGEOM_mod(BALIAS,DSTOT,MATER,KDET,
C     1    KSLAST,NBODY)

C	  CALL fsurf2(KS, A, B, C)
C	  RETURN
...
C	 CALL transfTRACK_mod(E ,X ,Y,Z,U,V,W,WGHT,
C     1    SP1,SP2,SP3,PAGE,KPAR,IBODY,MAT,ILB,IPOL,LAGE)
	 
C	 CALL transfPENELOPE_mod(EABS,C1,C2,WCC,WCR,DEN,
C     1    RDEN,E0STEP,DESOFT,SSOFT,NMS,NEGP, NMAT)
	 
C	 CALL transfPENGEOM_mod(BALIAS,DSTOT,MATER,KDET,
C     1    KSLAST,NBODY,LVERB)
...
C	  write(*,*) KSURF(NBODYS, NXG)
C  30    format('KSURF= ', i6)	  
