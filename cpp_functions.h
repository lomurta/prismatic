...
void transfCPU_to_GPU()
{
	...
	// gpuErrchk(cudaMemcpy(d_CJUMP1, CJUMP1_, sizeof(hd_CJUMP1)*pilhaPart, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpyToSymbol(dg_CJUMP0_, d_CJUMP0, sizeof(hd_CJUMP0) * pilhaPart));
...
void transfGPU_to_CPU()
{
	...
	// CJUMP0
	// gpuErrchk(cudaMemcpyFromSymbol(d_CJUMP0, dg_CJUMP0_, sizeof(hd_CJUMP0)*pilhaPart));
...
void memoryAllocGPU()
{
	...
	gpuErrchk(cudaMalloc((void **)&d_CJUMP0, sizeof(hd_CJUMP0) * pilhaPart));
...
void iniPRITRACK()
{
	...
	for (int J = 0; J < pilhaPart; J++)
	{
		for (int I = 1; I <= 3; I++)
		{
			h_CNT0_.DPRIM[J][I - 1] = 0.0e0;
			for (int K = 1; K <= 3; K++)
			{
				h_CNT0_.DSEC[J][I - 1][K - 1] = 0.0e0;
			}
		}
	}
	...
	for (int J = 0; J < pilhaPart; J++)
	{
		for (int I = 1; I <= 2; I++)
		{
			h_CNT0_.DAVW[J][I - 1] = 0.0e0;
			h_CNT0_.DAVA[J][I - 1] = 0.0e0;
			h_CNT0_.DAVE[J][I - 1] = 0.0e0;
		}
	}
	...
	/// cleans2GPU_(); //Limpa a pilha secundaria
...
void simPriTrack_G()
{
	...
	// seta o valor da quantidade de particulas primarias a simular
...
void simSecTrack_E()
{
	...
	// printf("quantidade de blocos %d", ceil(sizeTrack / blockSec.x));
	// exit(0);
	...
	gpuErrchk(cudaDeviceSynchronize());
		...
		g_step2_<<<gridSec, blockSec>>>(sizeTrack);
		gpuErrchk(cudaDeviceSynchronize());
		...
		g_showers_step5_E<<<gridSec, blockSec>>>(sizeTrack);
		gpuErrchk(cudaDeviceSynchronize());
	...
	// chama o contador de particulas passo 4
	g_showers_step6<<<gridSec, blockSec>>>(sizeTrack);
	// Aguarda o termino da simulação das particulas primarias enviadas
	gpuErrchk(cudaDeviceSynchronize());
...
void simSecTrack_G()
{
	...
	nTRACKS_.nFINISH = nTRACKS_.nSECTRACK_G;
	...
	// chamada do kernel para simulacao das particulas primarias
	g_showers_sec_G<<<gridSec, blockSec>>>(sizeTrack);
	// Aguarda o termino da simulação das particulas primarias enviadas
	gpuErrchk(cudaDeviceSynchronize());

	printf("g_showers_Sec_G\n");

	g_showers_step3_G<<<gridSec, blockSec>>>(sizeTrack);
	gpuErrchk(cudaDeviceSynchronize());
	...
	while (nTRACKS_.nFINISH > 0)
	{
		...
		g_jump2_G<<<gridSec, blockSec>>>(sizeTrack);
		gpuErrchk(cudaDeviceSynchronize());
		...
		g_showers_step4_G<<<gridSec, blockSec>>>(sizeTrack);
		gpuErrchk(cudaDeviceSynchronize());

		printf("g_showers_step4_G\n");

		g_knock2_G<<<gridSec, blockSec>>>(sizeTrack);
		gpuErrchk(cudaDeviceSynchronize());

		printf("g_knock2_G\n");

		g_showers_step5_G<<<gridSec, blockSec>>>(sizeTrack);
		gpuErrchk(cudaDeviceSynchronize());

		printf("g_showers_step5_G\n");
...
void simSecTrack_P()
{
	...
	nTRACKS_.nFINISH = nTRACKS_.nSECTRACK_P;
	...
	// chamada do kernel para simulacao das particulas primarias
	g_showers_sec_P<<<gridSec, blockSec>>>(sizeTrack);
	// Aguarda o termino da simulação das particulas primarias enviadas
	gpuErrchk(cudaDeviceSynchronize());

	printf("g_showers_Sec_G\n");

	g_showers_step3_P<<<gridSec, blockSec>>>(sizeTrack);
	gpuErrchk(cudaDeviceSynchronize());
	...
	while (nTRACKS_.nFINISH > 0)
	{
		...
		g_jump2_P<<<gridSec, blockSec>>>(sizeTrack);
		gpuErrchk(cudaDeviceSynchronize());
		...
		g_showers_step4_P<<<gridSec, blockSec>>>(sizeTrack);
		gpuErrchk(cudaDeviceSynchronize());

		printf("g_showers_step4_P\n");

		g_knock2_P<<<gridSec, blockSec>>>(sizeTrack);
		gpuErrchk(cudaDeviceSynchronize());

		printf("g_knock2_P\n");

		g_showers_step5_P<<<gridSec, blockSec>>>(sizeTrack);
		gpuErrchk(cudaDeviceSynchronize());

		printf("g_showers_step5_P\n");
