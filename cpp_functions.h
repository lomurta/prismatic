...
void rita02_(int &PDF, double &XLOW, double &XHIGH, int &N, int &NU, double &ERRM);

void ritai02_(int &PDF, double &XLOW, double &XHIGH, int &N, int &NU, double &ERRM);
...
void transfCNT0_GPU_to_CPU();

void transfCNT1_CPU_to_GPU();

void transfCNT1_GPU_to_CPU();

void transfCNTRL_CPU_to_GPU();

void transfCNTRL_GPU_to_CPU();
...
void transfnTRACKSGPU_to_CPU();

void transfnTRACKSCPU_to_GPU();

void transfstepsGPU_to_CPU();

void transfstepsCPU_to_GPU();

void transfstepsGPU_to_CPU(int step);
...
void peinit2_(double *EMAX, int &NMATER, FILE *IWR, int *INFO, char (*PMFILE)[100])
{
	...
	*TRACK_mod_.IPOL = 0;
	*TRACK_mod_.SP1 = 0.0e0;
	*TRACK_mod_.SP2 = 0.0e0;
	*TRACK_mod_.SP3 = 0.0e0;
	...
	for (int M = 1; M <= *PENELOPE_mod_.NMAT; M++)
	{
		...
		if (M == 1)
			strcpy(LIT, "st");
		if (M == 2)
			strcpy(LIT, "nd"); // LIT = "nd";
		if (M == 3)
			strcpy(LIT, "rd"); // LIT = "rd";
		if (M == 4)
			strcpy(LIT, "th"); // LIT = "th";
...
{ // OK
	...
	rita02_(PDF, XMIN, XMAX, N, NU, ERRM);
...
{ // OK
   ...
   **** Coeficientes de interpola��o e tabelas PDF s�o impressos em
	...
	*/
	...
	ritai02_(PDF, XLOW, XHIGH, N, NU, ERRM);
...
void relaxr2_(FILE *IRD, FILE *IWR, int *INFO)
{
  ...
  elemento. (Veja os coment�rios do cabe�alho na sub-rotina RELAX).
	...
	*/
	...
	char LSHELL[30][3] = {{"K"}, {"L1"}, {"L2"}, {"L3"}, {"M1"}, {"M2"}, {"M3"}, {"M4"}, {"M5"}, {"N1"}, {"N2"}, {"N3"}, {"N4"}, {"N5"}, {"N6"}, {"N7"}, {"O1"}, {"O2"}, {"O3"}, {"O4"}, {"O5"}, {"O6"}, {"O7"}, {"P1"}, {"P2"}, {"P3"}, {"P4"}, {"P5"}, {"Q1"}, {"X"}};
...
void esiar2_(int *M, FILE *IRD, FILE *IWR, int *INFO)
{
	...
	*/
	...
	int N;
	double DX;
	char CS5[16][6] = {{"CS-K "}, {"CS-L1"}, {"CS-L2"}, {"CS-L3"}, {"CS-M1"}, {"CS-M2"}, {"CS-M3"}, {"CS-M4"}, {"CS-M5"}, {"CS-N1"}, {"CS-N2"}, {"CS-N3"}, {"CS-N4"}, {"CS-N5"}, {"CS-N6"}, {"CS-N7"}};
  ...
  Refereno: M.J. Maron, 'Numerial Analysis: a Pratial Approah',
	...
	*/
	...
	for (int I = 1; I <= N1; I++)
	{
		...
		H = A[I - 1];
		HI = 1.0e0 / H;
		A[I - 1] = (HI / 6.0e0) * (SI * pow(X[I + 1 - 1], 3) - SI1 * pow(X[I - 1], 3)) + HI * (Y[I - 1] * X[I + 1 - 1] - Y[I + 1 - 1] * X[I - 1]) + (H / 6.0e0) * (SI1 * X[I - 1] - SI * X[I + 1 - 1]);
		B[I - 1] = (HI / 2.0e0) * (SI1 * pow(X[I - 1], 2) - SI * pow(X[I + 1 - 1], 2)) + HI * (Y[I + 1 - 1] - Y[I - 1]) + (H / 6.0e0) * (SI - SI1);
...
{ // REVISTO
  ...
  Argumentos de sa�da:
	...
	*CEIN01_.AMOL = pow(E / (E + REV), 2);
	...
	if (WCCM < WTHR)
	{ // No soft interactions
		WL = WTHR;
		WU = WCMAX;
		H0 = H0 + (1.0e0 / (*CEIN01_.EE - WU)) - (1.0e0 / (*CEIN01_.EE - WL)) - (1.0e0 / WU) + (1.0e0 / WL) + (1.0e0 - *CEIN01_.AMOL) * log(((*CEIN01_.EE - WU) * WL) / ((*CEIN01_.EE - WL) * WU)) / *CEIN01_.EE + *CEIN01_.AMOL * (WU - WL) / pow(*CEIN01_.EE, 2);

		H1 = H1 + log(WU / WL) + (*CEIN01_.EE / (*CEIN01_.EE - WU)) - (*CEIN01_.EE / (*CEIN01_.EE - WL)) + (2.0e0 - *CEIN01_.AMOL) * log((*CEIN01_.EE - WU) / (*CEIN01_.EE - WL)) + *CEIN01_.AMOL * (pow(WU, 2) - pow(WL, 2)) / (2.0e0 * pow(*CEIN01_.EE, 2));

		H2 = H2 + (2.0e0 - *CEIN01_.AMOL) * (WU - WL) + (WU * (2.0e0 * *CEIN01_.EE - WU) / (*CEIN01_.EE - WU)) - (WL * (2.0e0 * *CEIN01_.EE - WL) / (*CEIN01_.EE - WL)) + (3.0e0 - *CEIN01_.AMOL) * *CEIN01_.EE * log((*CEIN01_.EE - WU) / (*CEIN01_.EE - WL)) + *CEIN01_.AMOL * (pow(WU, 3) - pow(WL, 3)) / (3.0e0 * pow(*CEIN01_.EE, 2));
	}
	else
	{
		if (WCCM > WCMAX)
		{
			WL = WTHR;
			WU = WCMAX;
			S0 = S0 + (1.0e0 / (*CEIN01_.EE - WU)) - (1.0e0 / (*CEIN01_.EE - WL)) - (1.0e0 / WU) + (1.0e0 / WL) + (1.0e0 - *CEIN01_.AMOL) * log(((*CEIN01_.EE - WU) * WL) / ((*CEIN01_.EE - WL) * WU)) / *CEIN01_.EE + *CEIN01_.AMOL * (WU - WL) / pow(*CEIN01_.EE, 2);

			S1 = S1 + log(WU / WL) + (*CEIN01_.EE / (*CEIN01_.EE - WU)) - (*CEIN01_.EE / (*CEIN01_.EE - WL)) + (2.0e0 - *CEIN01_.AMOL) * log((*CEIN01_.EE - WU) / (*CEIN01_.EE - WL)) + *CEIN01_.AMOL * (pow(WU, 2) - pow(WL, 2)) / (2.0e0 * pow(*CEIN01_.EE, 2));

			S2 = S2 + (2.0e0 - *CEIN01_.AMOL) * (WU - WL) + (WU * (2.0e0 * *CEIN01_.EE - WU) / (*CEIN01_.EE - WU)) - (WL * (2.0e0 * *CEIN01_.EE - WL) / (*CEIN01_.EE - WL)) + (3.0e0 - *CEIN01_.AMOL) * *CEIN01_.EE * log((*CEIN01_.EE - WU) / (*CEIN01_.EE - WL)) + *CEIN01_.AMOL * (pow(WU, 3) - pow(WL, 3)) / (3.0e0 * pow(*CEIN01_.EE, 2));
		}
		else
		{
			WL = WCCM;
...
{ // OK
	...
	*/
	...
	resultado = (1.0e0 + pow(W / (*CEIN01_.EE - W), 2) - (1.0e0 - *CEIN01_.AMOL) * (W / (*CEIN01_.EE - W)) + *CEIN01_.AMOL * pow(W / *CEIN01_.EE, 2)) * DWDMU * pow(RMU, *CEIN01_.MOM) / pow(W, 2);
...
L3:;
	...
	for (int I = 1; I <= CEIN_.NOSC[*M - 1]; I++)
	{
		CPIN00_.SPH0[I - 1] = 0.0e0;
		CPIN00_.SPH1[I - 1] = 0.0e0;
		CPIN00_.SPH2[I - 1] = 0.0e0;
		CPIN00_.SPS0[I - 1] = 0.0e0;
		CPIN00_.SPS1[I - 1] = 0.0e0;
		CPIN00_.SPS2[I - 1] = 0.0e0;
		CPIN00_.SPT0[I - 1] = 0.0e0;
		CPIN00_.SPT1[I - 1] = 0.0e0;
		CPIN00_.SPT2[I - 1] = 0.0e0;
	}
...
void pinat12_(double &E, double &UK, double &WK, double &DELTA, double &WCCM, double &H0, double &H1, double &H2, double &S0, double &S1, double &S2, double &R0, double &R1, double &R2)
{
	...
	*CPIN01_.BHA4 = AMOL * pow(GAM - 1.0e0, 2) / G12;
	...
	if (WCCM < WTHR)
	{ // Sem interacoes macias
		WL = WTHR;
		WU = WCMAX;
		H0 = H0 + (1.0e0 / WL) - (1.0e0 / WU) - *CPIN01_.BHA1 * log(WU / WL) / E + *CPIN01_.BHA2 * (WU - WL) / pow(E, 2) - *CPIN01_.BHA3 * (pow(WU, 2) - pow(WL, 2)) / (2.0e0 * pow(E, 3)) + *CPIN01_.BHA4 * (pow(WU, 3) - pow(WL, 3)) / (3.0e0 * pow(E, 4));

		H1 = H1 + log(WU / WL) - *CPIN01_.BHA1 * (WU - WL) / E + *CPIN01_.BHA2 * (pow(WU, 2) - pow(WL, 2)) / (2.0e0 * pow(E, 2)) - *CPIN01_.BHA3 * (pow(WU, 3) - pow(WL, 3)) / (3.0e0 * pow(E, 3)) + *CPIN01_.BHA4 * (pow(WU, 4) - pow(WL, 4)) / (4.0e0 * pow(E, 4));

		H2 = H2 + WU - WL - *CPIN01_.BHA1 * (pow(WU, 2) - pow(WL, 2)) / (2.0e0 * E) + *CPIN01_.BHA2 * (pow(WU, 3) - pow(WL, 3)) / (3.0e0 * pow(E, 2)) - *CPIN01_.BHA3 * (pow(WU, 4) - pow(WL, 4)) / (4.0e0 * pow(E, 3)) + *CPIN01_.BHA4 * (pow(WU, 5) - pow(WL, 5)) / (5.0e0 * pow(E, 4));
	}
	else
	{
		if (WCCM > WCMAX)
		{
			WL = WTHR;
			WU = WCMAX;
			S0 = S0 + (1.0e0 / WL) - (1.0e0 / WU) - *CPIN01_.BHA1 * log(WU / WL) / E + *CPIN01_.BHA2 * (WU - WL) / pow(E, 2) - *CPIN01_.BHA3 * (pow(WU, 2) - pow(WL, 2)) / (2.0e0 * pow(E, 3)) + *CPIN01_.BHA4 * (pow(WU, 3) - pow(WL, 3)) / (3.0e0 * pow(E, 4));

			S1 = S1 + log(WU / WL) - *CPIN01_.BHA1 * (WU - WL) / E + *CPIN01_.BHA2 * (pow(WU, 2) - pow(WL, 2)) / (2.0e0 * pow(E, 2)) - *CPIN01_.BHA3 * (pow(WU, 3) - pow(WL, 3)) / (3.0e0 * pow(E, 3)) + *CPIN01_.BHA4 * (pow(WU, 4) - pow(WL, 4)) / (4.0e0 * pow(E, 4));

			S2 = S2 + WU - WL - *CPIN01_.BHA1 * (pow(WU, 2) - pow(WL, 2)) / (2.0e0 * E) + *CPIN01_.BHA2 * (pow(WU, 3) - pow(WL, 3)) / (3.0e0 * pow(E, 2)) - *CPIN01_.BHA3 * (pow(WU, 4) - pow(WL, 4)) / (4.0e0 * pow(E, 3)) + *CPIN01_.BHA4 * (pow(WU, 5) - pow(WL, 5)) / (5.0e0 * pow(E, 4));
		}
		else
		{
			WL = WCCM;
	...
	*/
	...
	else
	{
		...
		for (int I = IL; IL <= IU; IL++)
		{
			...
			if (I == IU)
				X2 = XUU;
			SUMP = X2 * (A[I - 1] + X2 * ((B[I - 1] / 2) + X2 * ((C[I - 1] / 3) + X2 * D[I - 1] / 4))) - X1 * (A[I - 1] + X1 * ((B[I - 1] / 2) + X1 * ((C[I - 1] / 3) + X1 * D[I - 1] / 4)));
			SUM = SUM + SUMP;
		}
	}
...
void pbrat2_(double &E, double &WCRM, double &XH0, double &XH1, double &XH2, double &XS1, double &XS2, int *M)
{
  ...
  Argumentos de sa�da:
	...
	*CEGRID_.XEK = *CEGRID_.XE - *CEGRID_.KE;
	...
	double FPOS = 1.0e0 - exp(-T * (1.2359e-1 - T * (6.1274e-2 - T * (3.1516e-2 - T * (7.7446e-3 - T * (1.0595e-3 - T * (7.0568e-5 - T * 1.8080e-6)))))));
	FACT = FACT * FPOS;
...
void panat2_(double &E, double &TXS)
{
  ...
  Argumento de sa�da:
	...
	*/
	...
	double F1 = sqrt(F2);
	TXS = PIELR2 * ((GAM2 + 4.0e0 * GAM + 1.0e0) * log(GAM + F1) / F2 - (GAM + 3.0e0) / F1) / (GAM + 1.0e0);
...
L20:;
	...
	for (int KE = 1; KE <= NEGP; KE++)
	{
		...
		ritai02_(PDF, W0, W1, NPP, NU, ERRM);
	...
	L1:;
		...
		else
		{
			...
			RMUC = CEELDB_.XSE[*M - 1][KE - 1][II - 1] + (CI * RR / (pow(DPRO, 2) + (DPRO * CEELDB_.ASE[*M - 1][KE - 1][II - 1] + CEELDB_.BSE[*M - 1][KE - 1][II - 1] * RR) * RR)) * (CEELDB_.XSE[*M - 1][KE - 1][II + 1 - 1] - CEELDB_.XSE[*M - 1][KE - 1][II - 1]);
		}
...
L100:;
	...
	L2:;
		...
		else
		{
			...
			RMUC = CPELDB_.XSP[*M - 1][KE - 1][II - 1] + (CI * RR / (pow(DPRO, 2) + (DPRO * CPELDB_.ASP[*M - 1][KE - 1][II - 1] + CPELDB_.BSP[*M - 1][KE - 1][II - 1] * RR) * RR)) * (CPELDB_.XSP[*M - 1][KE - 1][II + 1 - 1] - CPELDB_.XSP[*M - 1][KE - 1][II - 1]);
		}
...
void graar2_(int *M, FILE *IRD, FILE *IWR, int *INFO)
{
	...
	*CGRA01_.NE = atoi(APOIO);
	...
	if (*INFO >= 2)
	{
		fprintf(IWR, "\n*** Rayleigh scattering.  NQ = %3d, NE= %4d, NQQ = %4d\n", NQ, *CGRA01_.NE, NQQ);
	}
	...
	{
		XS1 = CGRA01_.XSRA[*CGRA01_.NE - 1 - 1][*M - 1] + (CGRA01_.XSRA[*CGRA01_.NE - 1][*M - 1] - CGRA01_.XSRA[*CGRA01_.NE - 1 - 1][*M - 1]) * (log(*CEGRID_.EU) - log(ER[*CGRA01_.NE - 1 - 1])) / (log(ER[*CGRA01_.NE - 1]) - log(ER[*CGRA01_.NE - 1 - 1]));
	...
	*CGRA00_.Q2MAX = 0.0e0;
	...
	ritai02_(PDF, Q2MIN, *CGRA00_.Q2MAX, NPT, NU, ERRM);
		...
		if (Q2M > CRITA_.QTI[1 - 1])
		{
			...
			L1:;
				...
				for (int K = 1; K <= NIP; K++)
				{
					...
					FUN[K - 1] = CRITA_.DPACI[II - 1] * pow(1.0e0 + (CRITA_.AI[II - 1] + CRITA_.BI[II - 1] * ETAP) * ETAP, 2) / ((1.0e0 - CRITA_.BI[II - 1] * ETAP * ETAP) * CI * (CRITA_.QTI[II + 1 - 1] - CRITA_.QTI[II - 1]));
				}
...
double graaf22_(double Q2)
{
	...
	else if (Q2 > *CGRA02_.QQM)
		resultado = 0.0e0;
	else
	{
		QL = log(Q2);
		findi2_(CGRA02_.QQ, &QL, &NQW, &I);
		F2 = CGRA02_.AR[I - 1][*CGRA00_.MM - 1] + QL * (CGRA02_.BR[I - 1][*CGRA00_.MM - 1] + QL * (CGRA02_.CR[I - 1][*CGRA00_.MM - 1] + QL * CGRA02_.DR[I - 1][*CGRA00_.MM - 1]));
		resultado = exp(F2);
	}
...
void gphar2_(int *M, FILE *IRD, FILE *IWR, int *INFO)
{
  ...
  realizar o rastreamento de f�tons mais r�pido.
	*/
	...
	char CS5[17][6] = {{"total"}, {"CS-K "}, {"CS-L1"}, {"CS-L2"}, {"CS-L3"}, {"CS-M1"}, {"CS-M2"}, {"CS-M3"}, {"CS-M4"}, {"CS-M5"}, {"CS-N1"}, {"CS-N2"}, {"CS-N3"}, {"CS-N4"}, {"CS-N5"}, {"CS-N6"}, {"CS-N7"}};
...
L1:;
	...
	ECS = exp(CGRA01_.XSRA[II - 1][*M - 1] + (CGRA01_.XSRA[II + 1 - 1][*M - 1] - CGRA01_.XSRA[II - 1][*M - 1]) * (XELN - CGRA01_.ERA[II - 1]) / (CGRA01_.ERA[II + 1 - 1] - CGRA01_.ERA[II - 1])) / COMPOS_.VMOL[*M - 1];
...
Se��o .
	...
	*/
	...
	A = ALZ * ALZ;
	FC = A * (0.202059e0 - A * (0.03693e0 - A * (0.00835e0 - A * (0.00201e0 - A * (0.00049e0 - A * (0.00012e0 - A * 0.00003e0))))) + 1.0e0 / (A + 1.0e0));
...
void pmrdr2_()
{
	...
	char KWTITL[7] = "TITLE ";
	char KWKPAR[7] = "SKPAR ";
	char KWSENE[7] = "SENERG";
	char KWSPEC[7] = "SPECTR";
	char KWSPOL[7] = "SGPOL ";
	char KWSPOS[7] = "SPOSIT";
	char KWSBOX[7] = "SBOX  ";
	char KWSBOD[7] = "SBODY ";
	char KWSCON[7] = "SCONE ";
	char KWSREC[7] = "SRECTA";
	char KWPSFN[7] = "IPSFN ";
	char KWPSPL[7] = "IPSPLI";
	char KWRRSP[7] = "WGTWIN";
	char KWEMAX[7] = "EPMAX ";
	char KWMATF[7] = "MFNAME";
	char KWSIMP[7] = "MSIMPA";
	char KWGEOM[7] = "GEOMFN";
	char KWGPAR[7] = "PARINP";
	char KWSMAX[7] = "DSMAX ";
	char KWEABS[7] = "EABSB ";
	char KWIFOR[7] = "IFORCE";
	char KBRSPL[7] = "IBRSPL";
	char KXRSPL[7] = "IXRSPL";
	char KWNBE[7] = "NBE   ";
	char KWNBAN[7] = "NBANGL";
	char KWIDET[7] = "IMPDET";
	char KWIPSF[7] = "IDPSF ";
	char KWISPC[7] = "IDSPC ";
	char KWIFLN[7] = "IDFLNC";
	char KWDIAL[7] = "IDAGEL";
	char KWDIAF[7] = "IDAGEF";
	char KWIBOD[7] = "IDBODY";
	char KWIPAR[7] = "IDKPAR";
	char KWEDET[7] = "ENDETC";
	char KWESPC[7] = "EDSPC ";
	char KWEBOD[7] = "EDBODY";
	char KGRDXX[7] = "GRIDX ";
	char KGRDYY[7] = "GRIDY ";
	char KGRDZZ[7] = "GRIDZ ";
	char KGRDRR[7] = "GRIDR ";
	char KWRESU[7] = "RESUME";
	char KWDUMP[7] = "DUMPTO";
	char KWDMPP[7] = "DUMPP ";
	char KWRSEE[7] = "RSEED ";
	char KWNSIM[7] = "NSIMSH";
	char KWTIME[7] = "TIME  ";
	char KWCOMM[7] = "      ";
	...
	int KBSMAX, KB, ISOURC, NMATR, IHEAD, IP, NMATG, NBE, NBTH, NBPH, NDBOD, NDICH, NAGE, ITST, KPARD, NDECH, NBX, NBY, NBZ, IDOSE, NBR, IRESUM;
...
L31:;
	...
	L35_1:;
		...
		if ((KB < 1) || (KB > *PENGEOM_mod_.NBODY))
		{
			...
			printf("Incorrect body number 1. %s %s\n", KWORD, BUFFER);
			exit(0);
		}
	...
	L36:;
		...
		if ((KB < 1) || (KB > *PENGEOM_mod_.NBODY))
		{
			fprintf(IWR, "%s %s", KWORD, BUFFER);
			fprintf(IWR, "Incorrect body number.\n");
			printf("Incorrect body number 2. %s %s\n", KWORD, BUFFER);
			exit(0);
		}
	...
	L65:;
		...
		if (!strcmp(KWORD, KWIBOD))
		{
			...
			if ((KB < 1) || (KB > *PENGEOM_mod_.NBODY))
			{
				fprintf(IWR, "%s %s", KWORD, BUFFER);
				fprintf(IWR, "Incorrect body number.\n");
				printf("Incorrect body number 3. %s %s\n", KWORD, BUFFER);
				exit(0);
			}
...
void shower2_()
{
	...
	int IEXIT, NTRIAL, K, KEn, IBODYL, NCROSS, IDET, MATL, ICOL, LEFT;
	...
	if (*CSOUR0_.KPARP == 0)
	{ // não será implementado o tratamento para eletrons como particula primaria nesta versão do programas.
...
L302:;
	...
	{ // energia é muito baixa
		...
		if ((*TRACK_mod_.KPAR == 3) && (*TRACK_mod_.E > 1.0e-6))
...
double rand2_(double DUMMY)
{ // gerador de numeros aleatoriosr
	/*
...
L20:;
	...
	P10 = CBRANG_.BP1[1 - 1][IK - 1][IE - 1][M - 1] + BETA * (CBRANG_.BP1[2 - 1][IK - 1][IE - 1][M - 1] + BETA * (CBRANG_.BP1[3 - 1][IK - 1][IE - 1][M - 1] + BETA * CBRANG_.BP1[4 - 1][IK - 1][IE - 1][M - 1]));
	P11 = CBRANG_.BP1[1 - 1][IK + 1 - 1][IE - 1][M - 1] + BETA * (CBRANG_.BP1[2 - 1][IK + 1 - 1][IE - 1][M - 1] + BETA * (CBRANG_.BP1[3 - 1][IK + 1 - 1][IE - 1][M - 1] + BETA * CBRANG_.BP1[4 - 1][IK + 1 - 1][IE - 1][M - 1]));
	P1 = P10 + (RK - IK) * (P11 - P10);

	P20 = CBRANG_.BP2[1 - 1][IK - 1][IE - 1][M - 1] + BETA * (CBRANG_.BP2[2 - 1][IK - 1][IE - 1][M - 1] + BETA * (CBRANG_.BP2[3 - 1][IK - 1][IE - 1][M - 1] + BETA * CBRANG_.BP2[4 - 1][IK - 1][IE - 1][M - 1]));
	P21 = CBRANG_.BP2[1 - 1][IK + 1 - 1][IE - 1][M - 1] + BETA * (CBRANG_.BP2[2 - 1][IK + 1 - 1][IE - 1][M - 1] + BETA * (CBRANG_.BP2[3 - 1][IK + 1 - 1][IE - 1][M - 1] + BETA * CBRANG_.BP2[4 - 1][IK + 1 - 1][IE - 1][M - 1]));
...
L5:;
	...
	if (RR > 1.0e-16)
	{
		...
		XX = CGRA03_.QRA[M - 1][I - 1] + ((1.0e0 + CGRA03_.ARA[M - 1][I - 1] + CGRA03_.BRA[M - 1][I - 1]) * D * RR / (D * D + (CGRA03_.ARA[M - 1][I - 1] * D + CGRA03_.BRA[M - 1][I - 1] * RR) * RR)) * (CGRA03_.QRA[M - 1][I + 1 - 1] - CGRA03_.QRA[M - 1][I - 1]);
	}
	else
	{
	...
	*/
	...
	for (int I = 1; I <= CGCO_.NOSCCO[M - 1]; I++)
	{
		if (CGCO_.UICO[I - 1][M - 1] < E)
		{
			...
			PZOMC = CGCO_.FJ0[I - 1][M - 1] * (AUX - REV * CGCO_.UICO[I - 1][M - 1]) / (REV * sqrt(AUX + AUX + pow(CGCO_.UICO[I - 1][M - 1], 2)));
			if (PZOMC > 0.0e0)
...
void pmwrt2_(int ICLOSE)
{
	...
	*CDUMP_.LDUMP = false;
	...
	char penmain_res[50];
	sprintf(penmain_res, "penmain-res2_%d.dat", (int)*CNTRL_.SHN);

	FILE *IWR2 = fopen(penmain_res, "w");
	if (IWR2 == NULL)
	{
...
void dosew2_(double &SHN, double &TSIM, FILE *IWR)
{
	...
	{ // Cilindro
		...
		{
			XYZ = CDOSE3_.DXL[3 - 1] + (I3 - 0.5e0) * CDOSE3_.BDOSE[3 - 1];
			YAV = CDOSE1_.DOSE[I3 - 1][1 - 1][1 - 1];
			YAV2 = CDOSE1_.DOSE2[I3 - 1][1 - 1][1 - 1];
			YERR = 3.0e0 * sqrt(fabs(YAV2 - pow(YAV, 2) * DF));
			YAV = YAV * DF * CDOSE4_.VMASS[I3 - 1][1 - 1][1 - 1];
			YERR = YERR * DF * CDOSE4_.VMASS[I3 - 1][1 - 1][1 - 1];
...
void memoryAllocCPU()
{
	...
	*PENGEOM_mod_.LVERB = false;
	...
	CENANG_.LPDE = (int(*)[2][3])malloc(NBEM * 2 * 3 * sizeof(int));
	CENANG_.LPDA = (int(*)[NBTHM][3])malloc(NBPHM * NBTHM * 3 * sizeof(int));
	...
	CIMDET_.LEDEP = (int *)malloc(NIDM * sizeof(int));
	CIMDET_.LDIT = (int(*)[NIDM])malloc(NBEM2 * NIDM * sizeof(int));
	CIMDET_.LDIP = (int(*)[NBEM2][NIDM])malloc(3 * NBEM2 * NIDM * sizeof(int));
	CIMDET_.LFLT = (int(*)[NIDM])malloc(NBEM2 * NIDM * sizeof(int));
	CIMDET_.LFLP = (int(*)[NBEM2][NIDM])malloc(3 * NBEM2 * NIDM * sizeof(int));
...
void transfCPU_to_GPU()
{
	...
	// gpuErrchk(cudaMemcpy(d_RSEED2_->ISEED1, h_RSEED2_.ISEED1, sizeof(int)*tamISEEDs, cudaMemcpyHostToDevice));
	// gpuErrchk(cudaMemcpy(d_RSEED2_->ISEED2, h_RSEED2_.ISEED2, sizeof(int)*tamISEEDs, cudaMemcpyHostToDevice));
	// gpuErrchk(cudaMemcpyToSymbol(dg_RSEED_, d_RSEED, sizeof(hd_RSEED)));*/
	// rand02_(1);
	gpuErrchk(cudaMemcpyToSymbol(dg_RSEED2_, &h_RSEED2_, sizeof(hd_RSEED)));
	...
	// TRACK_MOD
	/*	gpuErrchk(cudaMalloc((void **)&d_TRACK_mod, sizeof(hd_TRACK_MOD)));
		gpuErrchk(cudaMemcpy(&d_TRACK_mod->E,TRACK_mod_.E, sizeof(double), cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpy(&d_TRACK_mod->X,TRACK_mod_.X, sizeof(double), cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpy(&d_TRACK_mod->Y,TRACK_mod_.Y, sizeof(double), cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpy(&d_TRACK_mod->Z,TRACK_mod_.Z, sizeof(double), cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpy(&d_TRACK_mod->U,TRACK_mod_.U, sizeof(double), cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpy(&d_TRACK_mod->V,TRACK_mod_.V, sizeof(double), cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpy(&d_TRACK_mod->W,TRACK_mod_.W, sizeof(double), cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpy(&d_TRACK_mod->WGHT,TRACK_mod_.WGHT, sizeof(double), cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpy(&d_TRACK_mod->SP1,TRACK_mod_.SP1, sizeof(double), cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpy(&d_TRACK_mod->SP2,TRACK_mod_.SP2, sizeof(double), cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpy(&d_TRACK_mod->SP3,TRACK_mod_.SP3, sizeof(double), cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpy(&d_TRACK_mod->PAGE,TRACK_mod_.PAGE, sizeof(double), cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpy(&d_TRACK_mod->KPAR,TRACK_mod_.KPAR, sizeof(int), cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpy(&d_TRACK_mod->IBODY,TRACK_mod_.IBODY, sizeof(int), cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpy(d_TRACK_mod->ILB,TRACK_mod_.ILB, sizeof(int)*5, cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpy(&d_TRACK_mod->IPOL,TRACK_mod_.IPOL, sizeof(int), cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpy(&d_TRACK_mod->LAGE,TRACK_mod_.LAGE, sizeof(bool), cudaMemcpyHostToDevice));
		gpuErrchk(cudaMemcpyToSymbol(dg_TRACK_mod_, d_TRACK_mod, sizeof(hd_TRACK_MOD)));*/

	// QBODY

	gpuErrchk(cudaMemcpy(d_QBODY->KBODY, QBODY_.KBODY, sizeof(int) * NXG * NB, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_QBODY->KBOMO, QBODY_.KBOMO, sizeof(int) * NB, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpyToSymbol(dg_QBODY_, d_QBODY, sizeof(hd_QBODY)));

	// CDOSE1

	gpuErrchk(cudaMemcpy(d_CDOSE1->DOSE, CDOSE1_.DOSE, sizeof(double) * NDZM * NDYM * NDXM, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CDOSE1->DOSE2, CDOSE1_.DOSE2, sizeof(double) * NDZM * NDYM * NDXM, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CDOSE1->DOSEP, CDOSE1_.DOSEP, sizeof(double) * NDZM * NDYM * NDXM, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CDOSE1->LDOSE, CDOSE1_.LDOSE, sizeof(int) * NDZM * NDYM * NDXM, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(&d_CDOSE1->KDOSE, CDOSE1_.KDOSE, sizeof(int), cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpyToSymbol(dg_CDOSE1_, d_CDOSE1, sizeof(hd_CDOSE1)));

	// CDOSE2

	gpuErrchk(cudaMemcpy(d_CDOSE2->DDOSE, CDOSE2_.DDOSE, sizeof(double) * NDZM, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CDOSE2->DDOSE2, CDOSE2_.DDOSE2, sizeof(double) * NDZM, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CDOSE2->DDOSEP, CDOSE2_.DDOSEP, sizeof(double) * NDZM, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CDOSE2->LDDOSE, CDOSE2_.LDDOSE, sizeof(int) * NDZM, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpyToSymbol(dg_CDOSE2_, d_CDOSE2, sizeof(hd_CDOSE2)));

	// CDOSE3

	gpuErrchk(cudaMemcpy(d_CDOSE3->DXL, CDOSE3_.DXL, sizeof(double) * 3, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CDOSE3->DXU, CDOSE3_.DXU, sizeof(double) * 3, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CDOSE3->BDOSE, CDOSE3_.BDOSE, sizeof(double) * 3, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CDOSE3->RBDOSE, CDOSE3_.RBDOSE, sizeof(double) * 3, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CDOSE3->NDB, CDOSE3_.NDB, sizeof(int) * 3, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpyToSymbol(dg_CDOSE3_, d_CDOSE3, sizeof(hd_CDOSE3)));
	...
	/*gpuErrchk(cudaMemcpy(d_CNT0->PRIM,CNT0_.PRIM, sizeof(double)*3, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CNT0->PRIM2,CNT0_.PRIM2, sizeof(double)*3, cudaMemcpyHostToDevice));

	gpuErrchk(cudaMemcpy(d_CNT0->DPRIM,h_CNT0_.DPRIM, sizeof(double)*pilhaPart*3, cudaMemcpyHostToDevice));

	gpuErrchk(cudaMemcpy(d_CNT0->SEC,CNT0_.SEC, sizeof(double)*3*3, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CNT0->SEC2,CNT0_.SEC2, sizeof(double)*3*3, cudaMemcpyHostToDevice));

	gpuErrchk(cudaMemcpy(d_CNT0->DSEC,h_CNT0_.DSEC, sizeof(double)*pilhapart*3*3, cudaMemcpyHostToDevice));

	gpuErrchk(cudaMemcpy(d_CNT0->AVW,CNT0_.AVW, sizeof(double)*2, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CNT0->AVW2,CNT0_.AVW2, sizeof(double)*2, cudaMemcpyHostToDevice));

	gpuErrchk(cudaMemcpy(d_CNT0->DAVW,h_CNT0_.DAVW, sizeof(double)*pilhaPart*2, cudaMemcpyHostToDevice));

	gpuErrchk(cudaMemcpy(d_CNT0->AVA,CNT0_.AVA, sizeof(double)*2, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CNT0->AVA2,CNT0_.AVA2, sizeof(double)*2, cudaMemcpyHostToDevice));

	gpuErrchk(cudaMemcpy(d_CNT0->DAVA,h_CNT0_.DAVA, sizeof(double)*pilhaPart*2, cudaMemcpyHostToDevice));

	gpuErrchk(cudaMemcpy(d_CNT0->AVE,CNT0_.AVE, sizeof(double)*2, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CNT0->AVE2,CNT0_.AVE2, sizeof(double)*2, cudaMemcpyHostToDevice));

	gpuErrchk(cudaMemcpy(d_CNT0->DAVE,h_CNT0_.DAVE, sizeof(double)*pilhaPart*2, cudaMemcpyHostToDevice));

	gpuErrchk(cudaMemcpyToSymbol(dg_CNT0_, d_CNT0, sizeof(hd_CNT0)));*/
	...
	gpuErrchk(cudaMemcpyToSymbol(dg_CNT3_, d_CNT3, sizeof(hd_CNT3)));

	// CNT4

	gpuErrchk(cudaMemcpy(&d_CNT4->RLAST, CNT4_.RLAST, sizeof(double), cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(&d_CNT4->RWRITE, CNT4_.RWRITE, sizeof(double), cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CNT4->IDCUT, CNT4_.IDCUT, sizeof(int) * NIDM, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CNT4->KKDI, CNT4_.KKDI, sizeof(int) * NIDM * 3, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CNT4->IPSF, CNT4_.IPSF, sizeof(int) * NIDM, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(&d_CNT4->NID, CNT4_.NID, sizeof(int), cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(&d_CNT4->NPSFO, CNT4_.NPSFO, sizeof(int), cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(&d_CNT4->IPSFO, CNT4_.IPSFO, sizeof(int), cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpyToSymbol(dg_CNT4_, d_CNT4, sizeof(hd_CNT4)));
	...
	gpuErrchk(cudaMemcpy(&d_CNT6->LDOSEM, CNT6_.LDOSEM, sizeof(bool), cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpyToSymbol(dg_CNT6_, d_CNT6, sizeof(hd_CNT6)));

	// CNTRL

	// gpuErrchk(cudaMemcpy(&d_CNTRL->TSIM,CNTRL_.TSIM, sizeof(double), cudaMemcpyHostToDevice));
	// gpuErrchk(cudaMemcpy(&d_CNTRL->TSEC,CNTRL_.TSEC, sizeof(double), cudaMemcpyHostToDevice));
	// gpuErrchk(cudaMemcpy(&d_CNTRL->TSECA,CNTRL_.TSECA, sizeof(double), cudaMemcpyHostToDevice));
	// gpuErrchk(cudaMemcpy(&d_CNTRL->TSECAD,CNTRL_.TSECAD, sizeof(double), cudaMemcpyHostToDevice));
	// gpuErrchk(cudaMemcpy(&d_CNTRL->CPUT0,CNTRL_.CPUT0, sizeof(double), cudaMemcpyHostToDevice));
	...
	gpuErrchk(cudaMemcpy(d_CSPGEO->DSMAX, CSPGEO_.DSMAX, sizeof(double) * NB, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpy(d_CSPGEO->EABSB, CSPGEO_.EABSB, sizeof(double) * NB * 3, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpyToSymbol(dg_CSPGEO_, d_CSPGEO, sizeof(hd_CSPGEO)));
...
void transfGPU_to_CPU()
{
	...
	// TRACK_MOD
	/*	gpuErrchk(cudaMemcpyFromSymbol(d_TRACK_mod, dg_TRACK_mod_, sizeof(hd_TRACK_MOD)));
		gpuErrchk(cudaMemcpy(TRACK_mod_.E, &d_TRACK_mod->E,sizeof(double), cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(TRACK_mod_.X,&d_TRACK_mod->X, sizeof(double), cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(TRACK_mod_.Y,&d_TRACK_mod->Y, sizeof(double), cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(TRACK_mod_.Z,&d_TRACK_mod->Z, sizeof(double), cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(TRACK_mod_.U,&d_TRACK_mod->U, sizeof(double), cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(TRACK_mod_.V,&d_TRACK_mod->V, sizeof(double), cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(TRACK_mod_.W, &d_TRACK_mod->W,sizeof(double), cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(TRACK_mod_.WGHT,&d_TRACK_mod->WGHT, sizeof(double), cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(TRACK_mod_.SP1,&d_TRACK_mod->SP1, sizeof(double), cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(TRACK_mod_.SP2,&d_TRACK_mod->SP2, sizeof(double), cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(TRACK_mod_.SP3,&d_TRACK_mod->SP3, sizeof(double), cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(TRACK_mod_.PAGE,&d_TRACK_mod->PAGE, sizeof(double), cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(TRACK_mod_.KPAR,&d_TRACK_mod->KPAR, sizeof(int), cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(TRACK_mod_.IBODY,&d_TRACK_mod->IBODY, sizeof(int), cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(TRACK_mod_.ILB,d_TRACK_mod->ILB, sizeof(int)*5, cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(TRACK_mod_.IPOL,&d_TRACK_mod->IPOL, sizeof(int), cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(TRACK_mod_.LAGE,&d_TRACK_mod->LAGE, sizeof(bool), cudaMemcpyDeviceToHost));*/

	// QBODY
	/*gpuErrchk(cudaMemcpyFromSymbol(d_QBODY, dg_QBODY_, sizeof(hd_QBODY)));
	gpuErrchk(cudaMemcpy(QBODY_.KBODY,d_QBODY->KBODY, sizeof(int)*NXG*NB, cudaMemcpyDeviceToHost));
	gpuErrchk(cudaMemcpy(QBODY_.KBOMO, d_QBODY->KBOMO,sizeof(int)*NB, cudaMemcpyDeviceToHost));*/

	// CDOSE1
	gpuErrchk(cudaMemcpyFromSymbol(d_CDOSE1, dg_CDOSE1_, sizeof(hd_CDOSE1)));
	gpuErrchk(cudaMemcpy(CDOSE1_.DOSE, d_CDOSE1->DOSE, sizeof(double) * NDZM * NDYM * NDXM, cudaMemcpyDeviceToHost));
	gpuErrchk(cudaMemcpy(CDOSE1_.DOSE2, d_CDOSE1->DOSE2, sizeof(double) * NDZM * NDYM * NDXM, cudaMemcpyDeviceToHost));
	gpuErrchk(cudaMemcpy(CDOSE1_.DOSEP, d_CDOSE1->DOSEP, sizeof(double) * NDZM * NDYM * NDXM, cudaMemcpyDeviceToHost));
	gpuErrchk(cudaMemcpy(CDOSE1_.LDOSE, d_CDOSE1->LDOSE, sizeof(int) * NDZM * NDYM * NDXM, cudaMemcpyDeviceToHost));
	// gpuErrchk(cudaMemcpy(CDOSE1_.KDOSE,&d_CDOSE1->KDOSE, sizeof(int), cudaMemcpyDeviceToHost));

	// CDOSE2
	gpuErrchk(cudaMemcpyFromSymbol(d_CDOSE2, dg_CDOSE2_, sizeof(hd_CDOSE2)));
	...
	//gpuErrchk(cudaMemcpy(CDOSE3_.DXL,d_CDOSE3->DXL, sizeof(double)*3, cudaMemcpyDeviceToHost));
	//gpuErrchk(cudaMemcpy(CDOSE3_.DXU,d_CDOSE3->DXU, sizeof(double)*3, cudaMemcpyDeviceToHost));
	//gpuErrchk(cudaMemcpy(CDOSE3_.BDOSE,d_CDOSE3->BDOSE, sizeof(double)*3, cudaMemcpyDeviceToHost));
	//gpuErrchk(cudaMemcpy(CDOSE3_.RBDOSE,d_CDOSE3->RBDOSE, sizeof(double)*3, cudaMemcpyDeviceToHost));
	//gpuErrchk(cudaMemcpy(CDOSE3_.NDB,d_CDOSE3->NDB, sizeof(int)*3, cudaMemcpyDeviceToHost));*/
	...
	/*	gpuErrchk(cudaMemcpyFromSymbol(d_CNT0, dg_CNT0_, sizeof(hd_CNT0)));
		gpuErrchk(cudaMemcpy(CNT0_.PRIM,d_CNT0->PRIM, sizeof(double)*3, cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(CNT0_.PRIM2,d_CNT0->PRIM2, sizeof(double)*3, cudaMemcpyDeviceToHost));
		//gpuErrchk(cudaMemcpy(CNT0_.DPRIM,d_CNT0->DPRIM, sizeof(double)*3, cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(CNT0_.SEC,d_CNT0->SEC, sizeof(double)*3*3, cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(CNT0_.SEC2,d_CNT0->SEC2, sizeof(double)*3*3, cudaMemcpyDeviceToHost));
		//gpuErrchk(cudaMemcpy(CNT0_.DSEC,d_CNT0->DSEC, sizeof(double)*3*3, cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(CNT0_.AVW,d_CNT0->AVW, sizeof(double)*2, cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(CNT0_.AVW2, d_CNT0->AVW2,sizeof(double)*2, cudaMemcpyDeviceToHost));
		//gpuErrchk(cudaMemcpy(CNT0_.DAVW,d_CNT0->DAVW, sizeof(double)*2, cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(CNT0_.AVA,d_CNT0->AVA, sizeof(double)*2, cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(CNT0_.AVA2,d_CNT0->AVA2, sizeof(double)*2, cudaMemcpyDeviceToHost));
		//gpuErrchk(cudaMemcpy(CNT0_.DAVA,d_CNT0->DAVA, sizeof(double)*2, cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(CNT0_.AVE,d_CNT0->AVE, sizeof(double)*2, cudaMemcpyDeviceToHost));
		gpuErrchk(cudaMemcpy(CNT0_.AVE2,d_CNT0->AVE2, sizeof(double)*2, cudaMemcpyDeviceToHost));
		//gpuErrchk(cudaMemcpy(CNT0_.DAVE,d_CNT0->DAVE, sizeof(double)*2, cudaMemcpyDeviceToHost));*/

	// CNT1
	transfCNT1_GPU_to_CPU();
	/*gpuErrchk(cudaMemcpyFromSymbol(d_CNT1, dg_CNT1_, sizeof(hd_CNT1)));
	...
	//gpuErrchk(cudaMemcpy(CNT1_.DEBO,d_CNT1->DEBO, sizeof(double)*NB, cudaMemcpyDeviceToHost));*/
	...
	// CNT4
	/*gpuErrchk(cudaMemcpyFromSymbol(d_CNT4, dg_CNT4_, sizeof(hd_CNT4)));
	gpuErrchk(cudaMemcpy(CNT4_.RLAST, &d_CNT4->RLAST,sizeof(double), cudaMemcpyDeviceToHost));
	gpuErrchk(cudaMemcpy(CNT4_.RWRITE,&d_CNT4->RWRITE, sizeof(double), cudaMemcpyDeviceToHost));
	gpuErrchk(cudaMemcpy(CNT4_.IDCUT,d_CNT4->IDCUT, sizeof(int)*NIDM, cudaMemcpyDeviceToHost));
	gpuErrchk(cudaMemcpy(CNT4_.KKDI,d_CNT4->KKDI, sizeof(int)*NIDM*3, cudaMemcpyDeviceToHost));
	gpuErrchk(cudaMemcpy(CNT4_.IPSF,d_CNT4->IPSF, sizeof(int)*NIDM, cudaMemcpyDeviceToHost));
	gpuErrchk(cudaMemcpy(CNT4_.NID,&d_CNT4->NID, sizeof(int), cudaMemcpyDeviceToHost));
	gpuErrchk(cudaMemcpy(CNT4_.NPSFO,&d_CNT4->NPSFO, sizeof(int), cudaMemcpyDeviceToHost));
	gpuErrchk(cudaMemcpy(CNT4_.IPSFO,&d_CNT4->IPSFO, sizeof(int), cudaMemcpyDeviceToHost));*/
	...
	// CNT6
	/*gpuErrchk(cudaMemcpyFromSymbol(d_CNT6, dg_CNT6_, sizeof(hd_CNT6)));
	...
	// gpuErrchk(cudaMemcpy(CNTRL_.TSECAD,&d_CNTRL->TSECAD, sizeof(double), cudaMemcpyDeviceToHost));
	// gpuErrchk(cudaMemcpy(CNTRL_.CPUT0,&d_CNTRL->CPUT0, sizeof(double), cudaMemcpyDeviceToHost));
	...
	//gpuErrchk(cudaMemcpy(CSPGEO_.DSMAX,d_CSPGEO->DSMAX, sizeof(double)*NB, cudaMemcpyDeviceToHost));
	gpuErrchk(cudaMemcpy(CSPGEO_.EABSB,d_CSPGEO->EABSB, sizeof(double)*NB*3, cudaMemcpyDeviceToHost));*/
...
void memoryAllocGPU()
{
	...
	gpuErrchk(cudaMalloc((void **)&d_RSEED2_, sizeof(hd_RSEED)));
	...
	gpuErrchk(cudaMalloc((void **)&d_QBODY, sizeof(hd_QBODY)));
	gpuErrchk(cudaMalloc((void **)&d_CDOSE1, sizeof(hd_CDOSE1)));
	gpuErrchk(cudaMalloc((void **)&d_CDOSE2, sizeof(hd_CDOSE2)));
	gpuErrchk(cudaMalloc((void **)&d_CDOSE3, sizeof(hd_CDOSE3)));
	gpuErrchk(cudaMalloc((void **)&d_CDOSE4, sizeof(hd_CDOSE4)));
	gpuErrchk(cudaMalloc((void **)&d_CRNDG3, sizeof(hd_CRNDG3)));
	gpuErrchk(cudaMalloc((void **)&d_CNT0, sizeof(hd_CNT0)));
	gpuErrchk(cudaMalloc((void **)&d_CNT1, sizeof(hd_CNT1)));
	gpuErrchk(cudaMalloc((void **)&d_CNT3, sizeof(hd_CNT3)));
	gpuErrchk(cudaMalloc((void **)&d_CNT4, sizeof(hd_CNT4)));
	gpuErrchk(cudaMalloc((void **)&d_CNT5, sizeof(hd_CNT5)));
	gpuErrchk(cudaMalloc((void **)&d_CNT6, sizeof(hd_CNT6)));
	gpuErrchk(cudaMalloc((void **)&d_CNTRL, sizeof(hd_CNTRL)));
	...
	/*	h_CNT0_ =  (hd_CNT0*)malloc(sizeof(hd_CNT0)); //contador de resulatdos 0
		...
		h_CNT3_ =  (hd_CNT3*)malloc(sizeof(hd_CNT3)); //contador de resulatdos 3*/
...
void memoryFreeGPU()
{
	...
	gpuErrchk(cudaFree(d_QBODY));
	gpuErrchk(cudaFree(d_CDOSE1));
	gpuErrchk(cudaFree(d_CDOSE2));
	gpuErrchk(cudaFree(d_CDOSE3));
	gpuErrchk(cudaFree(d_CDOSE4));
	gpuErrchk(cudaFree(d_CRNDG3));
	gpuErrchk(cudaFree(d_CNT0));
	gpuErrchk(cudaFree(d_CNT1));
	gpuErrchk(cudaFree(d_CNT3));
	gpuErrchk(cudaFree(d_CNT4));
	gpuErrchk(cudaFree(d_CNT5));
	gpuErrchk(cudaFree(d_CNT6));
	gpuErrchk(cudaFree(d_CNTRL));
	gpuErrchk(cudaFree(d_CSPGEO));
	gpuErrchk(cudaFree(d_CXRSPL));

	gpuErrchk(cudaFree(d_SECTRACK_G));
	gpuErrchk(cudaFree(d_SECTRACK_E));
	gpuErrchk(cudaFree(d_SECTRACK_P));
	gpuErrchk(cudaFree(d_nTRACKS));
...
void plotdose2_()
{
	...
	char plotdose2[50];
	sprintf(plotdose2, "plotdose2_%d.dat", (int)*CNTRL_.SHN);

	FILE *IW = fopen(plotdose2, "w");
	if (IW == NULL)
	{
...
void transfSecTracksCPU_to_GPU()
{
	...
	// PRITRACK
	/*gpuErrchk(cudaMalloc((void **)&d_TRACK_mod, sizeof(hd_TRACK_MOD)*pilhaPart));
	gpuErrchk(cudaMemcpy(d_PRITRACK, PRITRACK, sizeof(hd_TRACK_MOD)*pilhaPart, cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpyToSymbol(dg_PRITRACK_, d_PRITRACK, sizeof(hd_TRACK_MOD)*pilhaPart));*/
...
void transfSecTracksGPU_to_CPU()
{
	...
	// PRITRACK_G
	/*	gpuErrchk(cudaMemcpyFromSymbol(d_PRITRACK, dg_PRITRACK_, sizeof(hd_TRACK_MOD)*pilhaPart));
...
void simSecTrack_E()
{
	...
	transfstepsGPU_to_CPU();

	while (nTRACKS_.nFINISH > 0)
	{
		if (h_steps.step20 == 1)
		{
			g_showers_step20_G<<<grid, block,2*blockSize*sizeof(int)>>>(sizeTrack);
			gpuErrchk(cudaDeviceSynchronize());
			transfstepsGPU_to_CPU();
		}
		if (h_steps.step2 == 1)
		...
		//dim3 grid2(1024 / block.x);

		g_showers_step6_E<<<grid, block, 2*blockSize*sizeof(int)>>>(sizeTrack);
		gpuErrchk(cudaDeviceSynchronize());

		

		/*gpuErrchk(cudaFuncSetSharedMemConfig(g_showers_step7_0_G, cudaSharedMemBankSizeEightByte));

		g_showers_step7_0_G<<<grid, block>>>(sizeTrack);
		gpuErrchk(cudaDeviceSynchronize());
		
		transfstepsGPU_to_CPU();*/

		/*if (h_steps.step7_1 == 1)
		{
			gpuErrchk(cudaFuncSetSharedMemConfig(g_showers_step7_1_G, cudaSharedMemBankSizeEightByte));

			g_showers_step7_1_G<<<grid, block>>>(sizeTrack);
			gpuErrchk(cudaDeviceSynchronize());
			transfstepsGPU_to_CPU();
		}*/
		...
		g_showers_step9_G<<<grid, block,2*blockSize*sizeof(int)>>>(sizeTrack);
		gpuErrchk(cudaDeviceSynchronize());

		g_showers_step10_E<<<grid, block, 2*blockSize*sizeof(int)>>>(sizeTrack);
		gpuErrchk(cudaDeviceSynchronize());
		...
		if (h_steps.step11 == 1)
		{
			...
			g_showers_step11_E<<<grid, block, 2*blockSize*sizeof(int)>>>(sizeTrack);
			gpuErrchk(cudaDeviceSynchronize());
			transfstepsGPU_to_CPU();
		}
		...
		if (h_steps.step12 == 1)
		{
			...
			g_showers_step12_E<<<grid, block, 2*blockSize*sizeof(int)>>>(sizeTrack);
			gpuErrchk(cudaDeviceSynchronize());
			transfstepsGPU_to_CPU();
		}
		...
		if (h_steps.step13 == 1)
		{
			...
			g_showers_step13_E<<<grid, block, 2*blockSize*sizeof(int)>>>(sizeTrack);
			gpuErrchk(cudaDeviceSynchronize());
			transfstepsGPU_to_CPU();
		}
		...
		if (h_steps.step14 == 1)
		{
			...
			g_showers_step14_E<<<grid, block, 2*blockSize*sizeof(int)>>>(sizeTrack);
			gpuErrchk(cudaDeviceSynchronize());
			transfstepsGPU_to_CPU();
		}
		...
		if (h_steps.step15 == 1)
		{
			...
			g_showers_step15_E<<<grid, block, 2*blockSize*sizeof(int)>>>(sizeTrack);
			gpuErrchk(cudaDeviceSynchronize());
			transfstepsGPU_to_CPU();
		}
		...
		if (h_steps.step18 == 1)
		{
			g_showers_step18_G<<<grid, block,2*blockSize*sizeof(int)>>>(sizeTrack);
			gpuErrchk(cudaDeviceSynchronize());
			transfstepsGPU_to_CPU();
		}
...
void simSecTrack_G()
{
	...
	while (nTRACKS_.nFINISH > 0)
	{
		...
		g_showers_step6_G<<<grid, block, 2*blockSize*sizeof(int)>>>(sizeTrack);
		gpuErrchk(cudaDeviceSynchronize());
		...
		g_showers_step10_G<<<grid, block, 2*blockSize*sizeof(int)>>>(sizeTrack);
		gpuErrchk(cudaDeviceSynchronize());
...
void simSecTrack_P()
{
	...
	while (nTRACKS_.nFINISH > 0)
	{
		...
		g_showers_step6_P<<<grid, block, 2*blockSize*sizeof(int)>>>(sizeTrack);
		gpuErrchk(cudaDeviceSynchronize());
		...
		g_showers_step10_P<<<grid, block, 2*blockSize*sizeof(int)>>>(sizeTrack);
		gpuErrchk(cudaDeviceSynchronize());
...
void cpyTracks_simular(int tipo, int tamPilha)
{
	...
	{ // fotons
		for (int I = (nTRACKS_.nSECTRACK_G - 1); I >= (nTRACKS_.nSECTRACK_G - tamPilha); I--)
		{
			vTrack_Simular.E[J] = SECTRACK_G.E[I];
			vTrack_Simular.X[J] = SECTRACK_G.X[I];
			vTrack_Simular.Y[J] = SECTRACK_G.Y[I];
			vTrack_Simular.Z[J] = SECTRACK_G.Z[I];
			vTrack_Simular.U[J] = SECTRACK_G.U[I];
			vTrack_Simular.V[J] = SECTRACK_G.V[I];
			vTrack_Simular.W[J] = SECTRACK_G.W[I];
			vTrack_Simular.WGHT[J] = SECTRACK_G.WGHT[I];
			vTrack_Simular.KPAR[J] = SECTRACK_G.KPAR[I];
			vTrack_Simular.IBODY[J] = SECTRACK_G.IBODY[I];
			vTrack_Simular.MAT[J] = SECTRACK_G.MAT[I];

			vTrack_Simular.ILB[1 - 1][J] = SECTRACK_G.ILB[1 - 1][I];
			vTrack_Simular.ILB[2 - 1][J] = SECTRACK_G.ILB[2 - 1][I];
			vTrack_Simular.ILB[3 - 1][J] = SECTRACK_G.ILB[3 - 1][I];
			vTrack_Simular.ILB[4 - 1][J] = SECTRACK_G.ILB[4 - 1][I];
			vTrack_Simular.ILB[5 - 1][J] = SECTRACK_G.ILB[5 - 1][I];
			...
			if (wIPOLI == 1)
			{
				vTrack_Simular.SP1[J] = SECTRACK_G.SP1[I];
				vTrack_Simular.SP2[J] = SECTRACK_G.SP2[I];
				vTrack_Simular.SP3[J] = SECTRACK_G.SP3[I];
				vTrack_Simular.IPOL[J] = SECTRACK_G.IPOL[I];
			}
			else
			{
				vTrack_Simular.SP1[J] = 0.0e0;
			...
			vTrack_Simular.PAGE[J] = SECTRACK_G.PAGE[I];

			J++;
		}
		nTRACKS_.nSECTRACK_G = nTRACKS_.nSECTRACK_G - tamPilha;
	}
	else if (tipo == 3)
	{ // protons
		for (int I = (nTRACKS_.nSECTRACK_P - 1); I >= (nTRACKS_.nSECTRACK_P - tamPilha); I--)
		{
			vTrack_Simular.E[J] = SECTRACK_P.E[I];
			vTrack_Simular.X[J] = SECTRACK_P.X[I];
			vTrack_Simular.Y[J] = SECTRACK_P.Y[I];
			vTrack_Simular.Z[J] = SECTRACK_P.Z[I];
			vTrack_Simular.U[J] = SECTRACK_P.U[I];
			vTrack_Simular.V[J] = SECTRACK_P.V[I];
			vTrack_Simular.W[J] = SECTRACK_P.W[I];
			vTrack_Simular.WGHT[J] = SECTRACK_P.WGHT[I];
			vTrack_Simular.KPAR[J] = SECTRACK_P.KPAR[I];
			vTrack_Simular.IBODY[J] = SECTRACK_P.IBODY[I];
			vTrack_Simular.MAT[J] = SECTRACK_P.MAT[I];

			vTrack_Simular.ILB[1 - 1][J] = SECTRACK_P.ILB[1 - 1][I];
			vTrack_Simular.ILB[2 - 1][J] = SECTRACK_P.ILB[2 - 1][I];
			vTrack_Simular.ILB[3 - 1][J] = SECTRACK_P.ILB[3 - 1][I];
			vTrack_Simular.ILB[4 - 1][J] = SECTRACK_P.ILB[4 - 1][I];
			vTrack_Simular.ILB[5 - 1][J] = SECTRACK_P.ILB[5 - 1][I];
			...
			if (wIPOLI == 1)
			{
				vTrack_Simular.SP1[J] = SECTRACK_P.SP1[I];
				vTrack_Simular.SP2[J] = SECTRACK_P.SP2[I];
				vTrack_Simular.SP3[J] = SECTRACK_P.SP3[I];
				vTrack_Simular.IPOL[J] = SECTRACK_P.IPOL[I];
			}
			else
			{
				vTrack_Simular.SP1[J] = 0.0e0;
			...
			vTrack_Simular.PAGE[J] = SECTRACK_P.PAGE[I];

			J++;
		}
		nTRACKS_.nSECTRACK_P = nTRACKS_.nSECTRACK_P - tamPilha;
	}
}
...
void transfCNT0_GPU_to_CPU()
{
	...
	gpuErrchk(cudaMemcpy(h_CNT0_.DPRIM, d_CNT0->DPRIM, sizeof(double) * pilhaPart * 3, cudaMemcpyDeviceToHost));

	gpuErrchk(cudaMemcpy(CNT0_.SEC, d_CNT0->SEC, sizeof(double) * 3 * 3, cudaMemcpyDeviceToHost));
	gpuErrchk(cudaMemcpy(CNT0_.SEC2, d_CNT0->SEC2, sizeof(double) * 3 * 3, cudaMemcpyDeviceToHost));

	gpuErrchk(cudaMemcpy(h_CNT0_.DSEC, d_CNT0->DSEC, sizeof(double) * pilhaPart * 3 * 3, cudaMemcpyDeviceToHost));

	gpuErrchk(cudaMemcpy(CNT0_.AVW, d_CNT0->AVW, sizeof(double) * 2, cudaMemcpyDeviceToHost));
	gpuErrchk(cudaMemcpy(CNT0_.AVW2, d_CNT0->AVW2, sizeof(double) * 2, cudaMemcpyDeviceToHost));

	gpuErrchk(cudaMemcpy(h_CNT0_.DAVW, d_CNT0->DAVW, sizeof(double) * pilhaPart * 2, cudaMemcpyDeviceToHost));

	gpuErrchk(cudaMemcpy(CNT0_.AVA, d_CNT0->AVA, sizeof(double) * 2, cudaMemcpyDeviceToHost));
	gpuErrchk(cudaMemcpy(CNT0_.AVA2, d_CNT0->AVA2, sizeof(double) * 2, cudaMemcpyDeviceToHost));

	gpuErrchk(cudaMemcpy(h_CNT0_.DAVA, d_CNT0->DAVA, sizeof(double) * pilhaPart * 2, cudaMemcpyDeviceToHost));

	gpuErrchk(cudaMemcpy(CNT0_.AVE, d_CNT0->AVE, sizeof(double) * 2, cudaMemcpyDeviceToHost));
	gpuErrchk(cudaMemcpy(CNT0_.AVE2, d_CNT0->AVE2, sizeof(double) * 2, cudaMemcpyDeviceToHost));

	gpuErrchk(cudaMemcpy(h_CNT0_.DAVE, d_CNT0->DAVE, sizeof(double) * pilhaPart * 2, cudaMemcpyDeviceToHost));
}
...
void transfCNTRL_GPU_to_CPU()
{
	...
	// CNTRL
	gpuErrchk(cudaMemcpyFromSymbol(d_CNTRL, dg_CNTRL_, sizeof(hd_CNTRL)));
	gpuErrchk(cudaMemcpy(h_vetN, d_CNTRL->N, sizeof(int) * NB, cudaMemcpyDeviceToHost));
}
...
int partition( int start, int end)
{
	...
	//int pivot = arr[start];
	double pivot = PRITRACK.E[start];

	int count = 0;


	for (int i = start + 1; i <= end; i++) {
		if (PRITRACK.E[i] <= pivot)
			count++;
	}
	...
	swap(PRITRACK.E[pivotIndex], PRITRACK.E[start]);
	/**double aux = PRITRACK.E[start];
	PRITRACK.E[start] = PRITRACK.E[pivotIndex];
	PRITRACK.E[pivotIndex] = aux;*/
	...
	while( (i < pivotIndex) && (j > pivotIndex)) {
		...
		while (PRITRACK.E[i] <= pivot) {
			i++;
		}

		while (PRITRACK.E[j] > pivot) {
			j--;
		}

		if ((i < pivotIndex) && (j > pivotIndex)) {
			swap(PRITRACK.E[i++], PRITRACK.E[j--]);
			/*double aux = PRITRACK.E[i++];
			PRITRACK.E[i++] = PRITRACK.E[j--];
			PRITRACK.E[j--] = aux;*/
		}
	}
...
void simPriTrack_G()
{
	...
	if (*CNTRL_.SHN < 1e4)
	{
	//	if ((1e4 - *CNTRL_.SHN) < (double)pilhaPart)
		//	{
				restante = (1e4 - *CNTRL_.SHN);
		//	}
	}
	else if (*CNTRL_.SHN < 1e5)
	{
	//	if ((1e5 - *CNTRL_.SHN) < (double)pilhaPart)
		//	{
				restante = (1e5 - *CNTRL_.SHN);
		//	}
	}
	else if (*CNTRL_.SHN < 1e6)
	{
	//	if ((1e6 - *CNTRL_.SHN) < (double)pilhaPart)
		//	{
				restante = (1e6 - *CNTRL_.SHN);
		//	}
	}
	else if (*CNTRL_.SHN < 1e7)
	{
	//	if ((1e7 - *CNTRL_.SHN) < (double)pilhaPart)
		//	{
				restante = (1e7 - *CNTRL_.SHN);
		//	}
	}
	else if (*CNTRL_.SHN < 1e8)
	{
	//	if ((1e8 - *CNTRL_.SHN) < (double)pilhaPart)
			//{
				restante = (1e8 - *CNTRL_.SHN);
			//}
	}
	else if (*CNTRL_.SHN < 1e9)
	{
		//if ((1e9 - *CNTRL_.SHN) < (double)pilhaPart)
		//	{
				restante = (1e9 - *CNTRL_.SHN);
		//	}
	}

	//restante = (*CNTRL_.DSHN - *CNTRL_.SHN);

	//printf("restante %lf\n", restante);


	if (restante >= (double)pilhaPart){
		iniPRITRACK(pilhaPart);
		nTRACKS_.nFINISH = pilhaPart;
		
	
		//printf("maior igual\n");
	}else{
		iniPRITRACK((int)restante);
		nTRACKS_.nFINISH = (int)restante;
		//printf("menor \n");
	}
	...
	//printf("iniciou a ordenacao \n");
	quickSort( 0, nTRACKS_.nFINISH-1);
	...
	g_showers_step0_G<<<grid, block>>>(sizeTrack);
	gpuErrchk(cudaDeviceSynchronize());
...
void transfnTRACKSCPU_to_GPU()
{
	...
	// gpuErrchk(cudaMemcpy(d_nTRACKS, &nTRACKS_, sizeof(hd_nTRACKS), cudaMemcpyHostToDevice));
	// gpuErrchk(cudaMemcpyToSymbol(dg_nTRACKS_, d_nTRACKS, sizeof(hd_nTRACKS)));
	gpuErrchk(cudaMemcpyToSymbol(dg_nTRACKS_, &nTRACKS_, sizeof(hd_nTRACKS)));
}
...
void transfnTRACKSGPU_to_CPU()
{
	...
	// nTRACKS
	gpuErrchk(cudaMemcpyFromSymbol(&nTRACKS_, dg_nTRACKS_, sizeof(hd_nTRACKS)));
	// gpuErrchk(cudaMemcpyFromSymbol(d_nTRACKS, dg_nTRACKS_, sizeof(hd_nTRACKS)));
	// gpuErrchk(cudaMemcpy(&nTRACKS_, d_nTRACKS, sizeof(hd_nTRACKS), cudaMemcpyDeviceToHost));
}
...
void transfstepsCPU_to_GPU()
{
	...
	// gpuErrchk(cudaMemcpy(d_steps, &h_steps_, sizeof(hd_nTRACKS), cudaMemcpyHostToDevice));
	gpuErrchk(cudaMemcpyToSymbol(dg_steps, &h_steps, sizeof(hd_steps)));
}
...
void transfstepsGPU_to_CPU()
{
	...
	// nTRACKS
	gpuErrchk(cudaMemcpyFromSymbol(&h_steps, dg_steps, sizeof(hd_steps)));
	// gpuErrchk(cudaMemcpy(&hsteps_, d_steps, sizeof(hd_nTRACKS), cudaMemcpyDeviceToHost));
}
...
void transfstepsGPU_to_CPU(int step)
{
	...
	{
	case 2:
		gpuErrchk(cudaMemcpyFromSymbol(&h_step2, dg_step2, sizeof(int)));
		break;

	case 12:
		gpuErrchk(cudaMemcpyFromSymbol(&h_step12, dg_step12, sizeof(int)));
		break;
	case 14:
		gpuErrchk(cudaMemcpyFromSymbol(&h_step14, dg_step14, sizeof(int)));
		break;

	case 15:
		gpuErrchk(cudaMemcpyFromSymbol(&h_step15, dg_step15, sizeof(int)));
		break;

	case 18:
		gpuErrchk(cudaMemcpyFromSymbol(&h_step18, dg_step18, sizeof(int)));
		break;

	case 19:
		gpuErrchk(cudaMemcpyFromSymbol(&h_step19, dg_step19, sizeof(int)));
		break;

	case 20:
		gpuErrchk(cudaMemcpyFromSymbol(&h_step20, dg_step20, sizeof(int)));
		break;
	}*/
}
...
void contGlobal(){
	...
	for (int KB = 1; KB <= *PENGEOM_mod_.NBODY; KB++)
	{
		...
		CNT1_.TDEBO2[KB - 1] = CNT1_.TDEBO2[KB - 1] + pow(CNT1_.TDEBO[KB - 1], 2);
	...
	for (int I = 1; I <= 3; I++)
	{
		...
		CNT0_.PRIM2[I - 1] = CNT0_.PRIM2[I - 1] + pow(CNT0_.PRIM[I - 1], 2);
		for (int K = 1; K <= 3; K++)
		{
		
			CNT0_.SEC2[I - 1][K - 1] = CNT0_.SEC2[I - 1][K - 1] + pow(CNT0_.SEC[I - 1][K - 1], 2);
		}
	}
	...
	for (int I = 1; I <= 2; I++)
	{
		...
		CNT0_.AVW2[I - 1] = CNT0_.AVW2[I - 1] + pow(CNT0_.AVW[I - 1], 2);
	
		CNT0_.AVA2[I - 1] = CNT0_.AVA2[I - 1] + pow(CNT0_.AVA[I - 1], 2);

		CNT0_.AVE2[I - 1] = CNT0_.AVE2[I - 1] + pow(CNT0_.AVE[I - 1], 2);
	}
