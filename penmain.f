...
C  ****  Photon simulation tables.
     ...
     2  ITLRA(NP2,MAXMAT),ITURA(NP2,MAXMAT)
	  ...
	  COMMON/CENANG/EL1,EU1,THL,THU,BSEC,RBS1,BSTH,RBSTH,BSPH,RBSPH,
     ...
     1  DIT(NIDM,NBEM2),DIT2(NIDM,NBEM2),DITP(NIDM,NBEM2),
     1  DIP(NIDM,NBEM2,3),DIP2(NIDM,NBEM2,3),DIPP(NIDM,NBEM2,3),
     1  FLT(NIDM,NBEM2),FLT2(NIDM,NBEM2),FLTP(NIDM,NBEM2),
     1  FLP(NIDM,NBEM2,3),FLP2(NIDM,NBEM2,3),FLPP(NIDM,NBEM2,3),
     1  AGEL(NIDM),AGEU(NIDM),BAGE(NIDM),RBAGE(NIDM),AGE(NIDM,NBEM2),
     1  AGE2(NIDM,NBEM2),AGEP(NIDM,NBEM2),LEDEP(NIDM),
     1  LDIT(NIDM,NBEM2),LDIP(NIDM,NBEM2,3),LFLT(NIDM,NBEM2),
     1  LFLP(NIDM,NBEM2,3),LAGEA(NIDM,NBEM2),IDCUT2(NIDM),NE3(NIDM),
     1  LLE2(NIDM),LLAGE(NIDM),NAGE(NIDM),NID2
...
C	 CALL PINaT1(E,UK,WK,DELTA,WCCM,H0,H1,H2,S0,S1,S2,R0,R1,R2)
     ...
     1  FLT,FLT2,FLTP,
     1  FLP,FLP2,FLPP,
     1  AGEL,AGEU,BAGE,RBAGE,AGE,
     1  AGE2,AGEP,LEDEP,LDIT,
     1  LDIP,LFLT,LFLP,
     1  LAGEA,IDCUT2,NE3,LLE2,LLAGE,
     1  NAGE,NID2)
	 
	  CALL transfcendet(EL4,EU4,BSE4,RBSE4,
     1  EDEP4,EDEP24,DET,
     1  NE4,LLE4,NID4)
	 
	  CALL transfcdose1(DOSE,DOSE2,DOSEP,LDOSE,KDOSE)
	 
	  CALL transfcdose2(DDOSE,DDOSE2,DDOSEP,LDDOSE)
	  
	  CALL transfcdose3(DXL,DXU,BDOSE,RBDOSE,NDB)
	 
      CALL transfcdose4(VMASS)
	  
	  CALL transfcjump1(ELAST1,ELAST2,MHINGE,KSOFTE,KSOFTI,KDELTA)
...
c	  CLOSE(IWR)	
	  ...
	  write(IWR,*) 'CIMDET'
	  WRITE(IWR,'(8E14.5)') EL2,EU2,BSE2,RBSE2,ET2,EDEP,EDEP2,EDEPP
	  WRITE(IWR,'(8E14.5)') DIT,DIT2,DITP,DIP,DIP2,DIPP
	  WRITE(IWR,'(8E14.5)') FLT,FLT2,FLTP,FLP,FLP2,FLPP
	  WRITE(IWR,'(8E14.5)') AGEL,AGEU,BAGE,RBAGE,AGE,AGE2,AGEP
	  WRITE(IWR,*) LEDEP,LDIT,LDIP,LFLT,LFLP,LAGEA
	  WRITE(IWR,'(8I10)') IDCUT2,NE3
	  WRITE(IWR,*) LLE2,LLAGE
	  WRITE(IWR,'(8I10)') NAGE,NID2
	  CLOSE(IWR)

	  OPEN(IWR,FILE='CENDET.txt')
	  write(IWR,*) 'CENDET'
	  WRITE(IWR,'(8E14.5)') EL4,EU4,BSE4,RBSE4,EDEP4,EDEP24,DET
	  WRITE(IWR,*) LLE4
...
c	  CLOSE(IWR)
	  ...
	  write(IWR,*) 'CDOSE2'
	  WRITE(IWR,'(8E14.5)') DDOSE,DDOSE2,DDOSEP
	  WRITE(IWR,*) LDDOSE
	  CLOSE(IWR)
	  
	  OPEN(IWR,FILE='CDOSE3.txt')
	  write(IWR,*) 'CDOSE3'
	  WRITE(IWR,'(8E14.5)') DXL,DXU,BDOSE,RBDOSE
	  WRITE(IWR,*) NDB
	  CLOSE(IWR)
...
c	  CLOSE(IWR)
	  ...
	  OPEN(IWR,FILE='CJUMP1.txt')
	  write(IWR,*) 'CJUMP1'
	  WRITE(IWR,'(8E14.5)') ELAST1,ELAST2
	  WRITE(IWR,'(8I10)') MHINGE,KSOFTE,KSOFTI,KDELTA
	  CLOSE(IWR)
	  
	  OPEN(IWR,FILE='SECST.txt')
	  write(IWR,*) 'SECST'
	  WRITE(IWR,'(8E14.5)') ES,XSS,YS,ZS,US,VS,WS
