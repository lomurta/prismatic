...
C  >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
      ...
      SAVE  ! Saves all items in the module.
C
C  ****  Maximum number of materials in the geometry.
C
      INTEGER*4, PARAMETER :: MAXMAT=10
C
C  ****  Simulation parameters (must be defined before calling the
C        initialisation subroutine PEINIT).
C
C  ----  Absorption energies, EABS(KPAR,MAT).
      DOUBLE PRECISION, DIMENSION (3,MAXMAT) :: EABS=50.0D0
C  ----  Electron/positron transport parameters.
      DOUBLE PRECISION, DIMENSION (MAXMAT) :: C1=0.01D0,C2=0.01D0
      DOUBLE PRECISION, DIMENSION (MAXMAT) :: WCC=1.0D2,WCR=1.0D2
C
C  ****  Size of the secondary stack (maximum number of particles that
C        can be stored).
      INTEGER*4, PARAMETER :: NMS=1000
C
C  ****  Energy interpolation, number of grid points.
      INTEGER*4, PARAMETER :: NEGP=200
C
C  ****  Global information on the material system (defined by
C        subroutine PEINIT).
C  ----  Number of materials present.
      INTEGER*4 :: NMAT
C  ----  Material densities and its reciprocals.
      DOUBLE PRECISION, DIMENSION (MAXMAT) :: DEN=1.0D0, RDEN=1.0D0
C
C  ****  Random-hinge slowing-down parameters (output from subroutines
C        JUMP and KNOCK).
C          E0STEP ... energy at the beginning of the segment,
C          DESOFT ... energy loss along the step,
C          SSOFT .... effective stopping power, = DESOFT/step_length.
      DOUBLE PRECISION :: E0STEP,DESOFT,SSOFT
C
...
C     COMMON/CJUMP1/ELAST1,ELAST2,MHINGE,KSOFTE,KSOFTI,KDELTA
C
...
C                       SUBROUTINE PEINIT
C  *********************************************************************
      SUBROUTINE PEINIT(EMAX,NMATER,IWR,INFO,PMFILE)
C
...
C    NMATER .... number of materials in the geometry.
C    PMFILE .... array of MAXMAT 20-character strings. The first NMATER
...
C             the output file,
C               INFO=1 (or less), minimal (composition data only).
...
C  WCC(M) and WCR(M). This information is introduced through the module
C  PENELOPE_mod, which has to be loaded before calling PEINIT.
C
      USE PENELOPE_mod
      USE TRACK_mod
C
...
C
	  ...
	  COMMON/CRNDG3/XX(NR),A(NR),B(NR),F(NR),KA(NR),NPM1
	  COMMON/CRITAA/XA(NM),AA(NM),BA(NM),FA(NM),IA(NM),NPM1A
      COMMON/CRITA/XT(NM),PAC(NM),DPAC(NM),A1(NM),B1(NM),IL(NM),IU(NM),
     1  NPM11
	  COMMON/CRITAN/CNORM
	  
C

	  CALL PEINIT2(EMAX,NMATER,IWR,INFO,PMFILE)
	  RETURN	
c	  GO TO 1234

      WRITE(IWR,2000)
 2000 FORMAT(/1X,34('*'),/1X,'**   PENELOPE  (version 2014)   **',
     1  /1X,34('*'))
C
      ISGAW=0
      IERSEC=0
C
      NMAT=NMATER
      DO M=1,NMAT
        IF(EABS(1,M).LT.49.999D0) THEN
          WRITE(IWR,2001) M,EABS(1,M)
 2001     FORMAT(/1X,'EABS(1,',I2,') =',1P,E11.4,' eV',
     1     /1X,'*** ERROR: electron absorption energy ',
     1      'cannot be less than 50 eV')
          STOP 'Electron absorption energy less than 50 eV.'
        ENDIF
        EABS0(1,M)=EABS(1,M)
        IF(EABS(2,M).LT.49.999D0) THEN
          WRITE(IWR,2002) M,EABS(2,M)
 2002     FORMAT(/1X,'EABS(2,',I2,') =',1P,E11.4,' eV',
     1     /1X,'*** ERROR: photon absorption energy ',
     2      'cannot be less than 50 eV')
          STOP 'Photon absorption energy less than 50 eV.'
        ENDIF
        EABS0(2,M)=EABS(2,M)
        IF(EABS(3,M).LT.49.999D0) THEN
          WRITE(IWR,2003) M,EABS(3,M)
 2003     FORMAT(/1X,'EABS(3,',I2,') =',1P,E11.4,' eV',
     1     /1X,'*** ERROR: positron absorption energy ',
     2      'cannot be less than 50 eV')
          STOP 'Positron absorption energy less than 50 eV.'
        ENDIF
        EABS0(3,M)=EABS(3,M)
      ENDDO
C
...
C
      ...
      ENDDO
 2004 FORMAT(/1X,'WARNING: EABS(',I1,',',I2,') has been modified.')
      IF(EMIN.LT.50.0D0) EMIN=50.0D0
C
      WRITE(IWR,2005) EMIN,EMAX
 2005 FORMAT(/1X,'EMIN =',1P,E11.4,' eV,  EMAX =',E11.4,' eV')
      IF(EMAX.LT.EMIN+10.0D0) STOP 'The energy interval is too narrow.'
      IF(NMAT.GT.MAXMAT) THEN
        WRITE(IWR,2006) NMAT,MAXMAT,NMAT
 2006   FORMAT(/1X,'*** PENELOPE cannot handle ',I2,' different mater',
     1  'ials.'/5X,'Edit the source file and change the parameter ',
     2  'MAXMAT = ',I2,' to MAXMAT = ',I2)
        STOP 'PEINIT. Too many materials.'
      ENDIF
      IF(INFO.GT.2) WRITE(IWR,2007)
 2007 FORMAT(/1X,'NOTE: 1 mtu = 1 g/cm**2')
C

      CALL EGRID2(EMIN,EMAX)  ! Defines the simulation energy grid.
	  CALL ESIa02  ! Initialises electron impact ionisation routines.
      CALL PSIa02  ! Initialises positron impact ionisation routines.
      CALL GPHa02  ! Initialises photoelectric routines.
	  CALL RELAX0  ! Initialises atomic relaxation routines.
      CALL RELAX02  ! Initialises atomic relaxation routines.
	  CALL RNDG30  ! Initialises the Gaussian sampling routine.
	  CALL RNDG302  ! Initialises the Gaussian sampling routine.c     
c	  CALL EGRID(EMIN,EMAX)  ! Defines the simulation energy grid.
c      CALL ESIa0  ! Initialises electron impact ionisation routines.
c      CALL PSIa0  ! Initialises positron impact ionisation routines.
c      CALL GPHa0  ! Initialises photoelectric routines.
c      CALL RELAX0  ! Initialises atomic relaxation routines.
c     CALL RNDG30  ! Initialises the Gaussian sampling routine.
C
      ...
      DO M=1,NMAT
        ...
        IF(M.EQ.3) LIT='rd'
        IF(M.GT.3) LIT='th'
        WRITE(IWR,2008) M,LIT
 2008   FORMAT(//1X,22('*')/1X,'**  ',I2,A2,' material   **',
     1    /1X,22('*'))
        WRITE(IWR,2009) PMFILE(M)
 2009   FORMAT(/1X,'Material data file: ',A)
...
C
        WRITE(IWR,2010)
 2010   FORMAT(/1X,'*** Simulation parameters:')
        WRITE(IWR,2011) EABS(1,M)
 2011   FORMAT(5X,'Electron absorption energy =',1P,E11.4,' eV')
        WRITE(IWR,2012) EABS(2,M)
 2012   FORMAT(7X,'Photon absorption energy =',1P,E11.4,' eV')
        WRITE(IWR,2013) EABS(3,M)
 2013   FORMAT(5X,'Positron absorption energy =',1P,E11.4,' eV')
C
...
C
        ...
        IF(WCR(M).LT.0.0D0) WRITE(IWR,2014)
 2014   FORMAT(1X,'*** Warning: soft radiative losses are switched off')
        WRITE(IWR,2015) C1(M),C2(M),WCC(M),MAX(WCR(M),10.0D0)
 2015   FORMAT(6X,'C1 =',1P,E11.4,',       C2 =',E11.4,/5X,'WCC =',
     1    E11.4,' eV,   WCR =',E11.4,' eV',/)
C
...
C
      ...
      PARAMETER (AVOG=6.02214129D23)  ! Avogadro's number
...
C  ****  Positron inelastic coll. and inner-shell ionisation tables.
      ...
      DIMENSION STFI(NO),STFO(NO),INOUT(NO)
C
...
C
      LNAME=' PENELOPE (v. 2014)  Material data file ...............'
      READ(IRD,'(A55)') NAME
...
C
 ...
 1006 FORMAT(' Number of oscillators =',I4)
 ...
 1007   FORMAT(I4,1P,3E16.8,2I4)
      ...
      EXPT=EXP(EXPT/ZT(M))
C
      IF(ABS(EXPT-EXPOT(M)).GT.1.0D-6*EXPOT(M)) THEN
        WRITE(26,*) 'EXPT      =',EXPT
...
C
 ...
 1106   FORMAT(1X,'Number of shells =',I4)
...
C
C  ****  Scaled bremsstrahlung x-section.
...
C  ****  Bremsstrahlung angular distribution.
...
c	        CALL EINaT2(EIT(I),WCCM,XH0,XH1,XH2,XS0,XS1,XS2,XT1,XT2,
c     1      DELTA,M)
	 
          STPC=(XS1+XH1)*VMOL(M)
          DIFMAX=MAX(DIFMAX,100.0D0*ABS(STPC-STPI)/STPI)
        ENDIF
      ENDDO
C
...
C		CALL SPLINE2(EITL,FL,A,B,C,D,0.0D0,0.0D0,NDATA)
      ELSE
        ICOR=0
      ENDIF
C
C  **** Classify inner and outer shells.
C
      NI=0
      NS=0
      DO IO=1,NO
...
C
      DO KO=1,NOSC(M)
        IZZ=KZ(M,KO)
        ISH=KS(M,KO)
        IF(IZZ.LT.3.OR.ISH.GT.16.OR.UI(M,KO).LT.50.0D0) THEN
          NI=NI+1
          IEIN(M,NI)=KO
          ISIE(KO)=-NI
          INOUT(NI)=0
          DO IEL=1,NELEM(M)
            IF(IZZ.EQ.IZ(M,IEL)) STFO(NI)=STF(M,IEL)
          ENDDO
        ELSE IF(ISH.LE.NSESI(IZZ)) THEN
          EBIN=EB(IZZ,ISH)
          IF(EBIN.GT.ECUTR(M)) THEN
            NS=NS+1
            IESI(M,NS)=KO
            ISIE(KO)=NS
            DO IEL=1,NELEM(M)
              IF(IZZ.EQ.IZ(M,IEL)) STFI(NS)=STF(M,IEL)
            ENDDO
          ELSE
            NI=NI+1
            IEIN(M,NI)=KO
...
c		CALL EINaT2(EE,WCCM,XH0,XH1,XH2,XS0,XS1,XS2,XT1,XT2,DELTA,M)
        STPC=(XS1+XH1)*VMOL(M)
        IF(ICOR.EQ.1) THEN
          EC=DLEMP(I)
...
C  ****  Inner shells
          IF(IO.GT.0) THEN
            ...
            IF(EBIN.LT.EE) THEN
              INDC=IESIF(IZZ)-1
              PCSI=EXP(XESI(INDC+I,ISH))*STFI(IO)
              IF(PCSI.GT.1.0D-35) THEN
                CALL EINaT1(EE,UI(M,KO),WRI(M,KO),0.0D0,WCCM,
     1            H0,H1,H2,S0,S1,S2,R0,R1,R2)
c	            CALL EINaT12(EE,UI(M,KO),WRI(M,KO),0.0D0,WCCM,
c     1            H0,H1,H2,S0,S1,S2,R0,R1,R2)
                STOT=F(M,KO)*(S0+H0)
                IF(STOT.GT.1.0D-35) THEN
                  DFERMI=(SES0(KO)+SEH0(KO))/STOT
                  PCSI=PCSI*DFERMI
                  XSESI(I,IO)=PCSI
                  RNFO=PCSI/(SES0(KO)+SEH0(KO))
                  STPSI=STPSI+(SES1(KO)+SEH1(KO))*RNFO
                  STRSI=STRSI+(SES2(KO)+SEH2(KO))*RNFO
                ELSE
                  XSESI(I,IO)=PCSI
                  STPSI=STPSI+PCSI*EBIN
                  STRSI=STRSI+PCSI*EBIN**2
                ENDIF
              ENDIF
            ENDIF
          ELSE
C  ****  Outer shells
            IO=-IO
            IF(INOUT(IO).EQ.1) THEN
              IZZ=KZ(M,KO)
              ISH=KS(M,KO)
              EBIN=EB(IZZ,ISH)
              INDC=IESIF(IZZ)-1
              PCSI=EXP(XESI(INDC+I,ISH))*STFO(IO)
              CALL EINaT1(EE,UI(M,KO),WRI(M,KO),0.0D0,WCCM,
     1          H0,H1,H2,S0,S1,S2,R0,R1,R2)
c	          CALL EINaT12(EE,UI(M,KO),WRI(M,KO),0.0D0,WCCM,
c     1            H0,H1,H2,S0,S1,S2,R0,R1,R2)
...
C
        ...
        IF(NESI(M).GT.0) THEN
          ...
          ENDDO
          SEISI(M,I)=LOG(MAX(SSIT*VMOL(M),1.0D-35))
        ELSE
          SEISI(M,I)=LOG(1.0D-35)
        ENDIF
        IF(SSIT.GT.1.0D-35) THEN
          DO IO=1,NESI(M)
            ESIAC(M,I,IO)=ESIAC(M,I,IO)/SSIT
          ENDDO
        ELSE
          DO IO=1,NESI(M)
            ESIAC(M,I,IO)=1.0D0
...
C
        STPSI=STPSI*VMOL(M)
        STPIN=STPIN*VMOL(M)
        FNORM=(CSTPE(M,I)-STPSI)/STPIN
C
        SINT=0.0D0
        IF(NEIN(M).EQ.0) STOP 'PHMATR. No outer shells?'
        DO IO=1,NEIN(M)
          IF(INOUT(IO).EQ.0) XSEIN(I,IO)=FNORM*XSEIN(I,IO)
          EINAC(M,I,IO)=SINT
          SINT=SINT+XSEIN(I,IO)
        ENDDO
        IF(SINT.GT.1.0D-35) THEN
          DO IO=1,NEIN(M)
            EINAC(M,I,IO)=EINAC(M,I,IO)/SINT
          ENDDO
        ELSE
          DO IO=1,NEIN(M)
            EINAC(M,I,IO)=1.0D0
        ...
        ENDIF
C
        SEHIN(M,I)=LOG(MAX(SINT*VMOL(M),1.0D-35))
        TSTRE(M,I)=(STRSI+FNORM*STRIN)*VMOL(M)
        W1E(M,I)=(XS1SI+FNORM*XS1IN)*VMOL(M)
        W2E(M,I)=(XS2SI+FNORM*XS2IN)*VMOL(M)
        T1EI(I)=(XT1SI+FNORM*XT1IN)*VMOL(M)
        T2EI(I)=(XT2SI+FNORM*XT2IN)*VMOL(M)
      ENDDO
C
...
c	      CALL PINaT2(EIT(I),WCCM,XH0,XH1,XH2,XS0,XS1,XS2,XT1,XT2,
c     1      DELTA,M)
          STPC=(XS1+XH1)*VMOL(M)
          DIFMAX=MAX(DIFMAX,100.0D0*ABS(STPC-STPI)/STPI)
        ENDIF
...
c		CALL PINaT2(EE,WCCM,XH0,XH1,XH2,XS0,XS1,XS2,XT1,XT2,DELTA,M)
        STPC=(XS1+XH1)*VMOL(M)
        IF(ICOR.EQ.1) THEN
          EC=DLEMP(I)
          CALL FINDI(EITL,EC,NDATA,J)
...
C  ****  Inner shells
          IF(IO.GT.0) THEN
            ...
            IF(EBIN.LT.EE) THEN
              INDC=IPSIF(IZZ)-1
              PCSI=EXP(XPSI(INDC+I,ISH))*STFI(IO)
              IF(PCSI.GT.1.0D-35) THEN
                CALL PINaT1(EE,UI(M,KO),WRI(M,KO),0.0D0,WCCM,
     1            H0,H1,H2,S0,S1,S2,R0,R1,R2)
c	            CALL PINaT12(EE,UI(M,KO),WRI(M,KO),0.0D0,WCCM,
c    1          H0,H1,H2,S0,S1,S2,R0,R1,R2)
                STOT=F(M,KO)*(S0+H0)
                IF(STOT.GT.1.0D-35) THEN
...
C  ****  Outer shells
            ...
            IF(INOUT(IO).EQ.1) THEN
              ...
              EBIN=EB(IZZ,ISH)
              INDC=IPSIF(IZZ)-1
              PCSI=EXP(XPSI(INDC+I,ISH))*STFO(IO)
              CALL PINaT1(EE,UI(M,KO),WRI(M,KO),0.0D0,WCCM,
     1          H0,H1,H2,S0,S1,S2,R0,R1,R2)
c	          CALL PINaT12(EE,UI(M,KO),WRI(M,KO),0.0D0,WCCM,
c     1          H0,H1,H2,S0,S1,S2,R0,R1,R2)
...
C
        ...
        IF(NPSI(M).GT.0) THEN
          ...
          ENDDO
          SPISI(M,I)=LOG(MAX(SSIT*VMOL(M),1.0D-35))
        ELSE
          SPISI(M,I)=LOG(1.0D-35)
        ENDIF
        IF(SSIT.GT.1.0D-35) THEN
          DO IO=1,NPSI(M)
            PSIAC(M,I,IO)=PSIAC(M,I,IO)/SSIT
          ENDDO
        ELSE
          DO IO=1,NPSI(M)
            PSIAC(M,I,IO)=1.0D0
...
C
        STPSI=STPSI*VMOL(M)
        STPIN=STPIN*VMOL(M)
        FNORM=(CSTPP(M,I)-STPSI)/STPIN
C
        SINT=0.0D0
        IF(NPIN(M).EQ.0) STOP 'PHMATR. No outer shells?'
        DO IO=1,NPIN(M)
          IF(INOUT(IO).EQ.0) XSPIN(I,IO)=FNORM*XSPIN(I,IO)
          PINAC(M,I,IO)=SINT
          SINT=SINT+XSPIN(I,IO)
        ENDDO
        IF(SINT.GT.1.0D-35) THEN
          DO IO=1,NPIN(M)
            PINAC(M,I,IO)=PINAC(M,I,IO)/SINT
          ENDDO
        ELSE
          DO IO=1,NPIN(M)
            PINAC(M,I,IO)=1.0D0
        ...
        ENDIF
C
        SPHIN(M,I)=LOG(MAX(SINT*VMOL(M),1.0D-35))
        TSTRP(M,I)=(STRSI+FNORM*STRIN)*VMOL(M)
        W1P(M,I)=(XS1SI+FNORM*XS1IN)*VMOL(M)
        W2P(M,I)=(XS2SI+FNORM*XS2IN)*VMOL(M)
        T1PI(I)=(XT1SI+FNORM*XT1IN)*VMOL(M)
        T2PI(I)=(XT2SI+FNORM*XT2IN)*VMOL(M)
      ENDDO
C
C  ****  Bremsstrahlung x-section for electrons.
C
      DIFMAX=0.0D0
      DO I=1,NDATA
...
c		write(*,*) I,ET(I),WCRM,XH0,XH1,XH2,XS1,XS2,M
        STPR=(XS1+XH1)*VMOL(M)
        IF(ICOR.EQ.1) THEN
          EC=DLEMP(I)
          CALL FINDI(EITL,EC,NDATA,J)
...
c		write(*,*) 'P XH0:',XH0,'VMOL ',VMOL(M),'FACT: ',FACT
        IF(IBREMS.EQ.1) THEN
...
C	  CALL SPLINE2(ET,F1,A,B,C,D,0.0D0,0.0D0,NEGP)
      RANGE(1,M,1)=1.0D-8
...
C  ****  Compton scattering and pair production.
 ...
 1114 FORMAT(/2X,'Energy',5X,'CS-Comp',5X,'CS-pair',3X,'CS-triplet',/3X,
...
C
      ...
      ENDDO
C
      LNAME=' PENELOPE (v. 2014)  End of material data file ........'
      READ(IRD,'(A55)') NAME
      IF(NAME.NE.LNAME) THEN
        WRITE(IWR,'(/1X,''I/O error. Corrupt material data file.'')')
...
C                       SUBROUTINE PEMATW
C  *********************************************************************
      SUBROUTINE PEMATW(IFILE,MFNAME)
C
C  This subroutine generates the material definition file, a part of the
C  input data file of PENELOPE.
C
C  When IFILE=1 the output file is given the name MFNAME, otherwise the
C  user is asked to provide the filename.
C
      USE PENELOPE_mod
      USE TRACK_mod
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER*4 (I-N)
      CHARACTER LASYMB*2,CH2*2,CH5*5,MFNAME*20,PFILE*80,NAME*62
      PARAMETER (A0B=5.2917721092D-9)  ! Bohr radius (cm)
      PARAMETER (HREV=27.21138505D0)  ! Hartree energy (eV)
      PARAMETER (AVOG=6.02214129D23)  ! Avogadro's number
...
C  ****  'Standard' energy grid.
      PARAMETER (NEGP1=12000)
      DIMENSION EIT(NEGP1),ES(NEGP1),ESS(NEGP1)
      DIMENSION XGP0(NEGP1),XGT0(NEGP1),PPE(NEGP1),PPT(NEGP1)
C
C  ****  Lowest negative valences of the elements.
C
      DIMENSION VAL(99)
      DATA VAL/    0,  0, -1, -2, -3, -2,  0,  0,  0,
...
C
      OPEN(3,FILE='./pdfiles/pdcompos.pen')
      DO I=1,15
        READ(3,'(A62)') NAME
      ENDDO
      DO K1=1,300
...
C        indicator.
        IF(NSHT(IZZ).EQ.0) THEN
          OPEN(3,FILE='./pdfiles/pdatconf.p14')
          DO J=1,22
            READ(3,'(A5)') CH5
          ENDDO
          NS=0
          IZZT=0
          DO J=1,150000
            READ(3,2005,END=905) IIZ,IS,CH2,CH5,IIF,EIE,CCP,GA1,GA2
 2005       FORMAT(I3,I4,1X,A2,1X,A5,I3,4E13.5)
            IF(IIZ.EQ.IZZ) THEN
              NS=NS+1
              IF(NS.GT.30) THEN
                WRITE(6,'(/1X,''NS ='',I4)') NS
              ...
              IZZT=IZZT+IIF
              EB(IZZ,IS)=EIE
              IF(GA2.GT.0.0D0) THEN
                ALW(IZZ,IS)=GA2
              ELSE IF(GA1.GT.0.0D0) THEN
                ALW(IZZ,IS)=GA1
              ELSE
                ALW(IZZ,IS)=0.0D0
              ENDIF
...
C  ****  Oscillators are sorted by increasing ionisation energies.
      DO I=1,NOS-1
        DO J=I+1,NOS
...
C
     ...
     1  E12.5,'' eV'',/2X,''(for insulators, these quantiti'',
...
C
      ...
      JBM=1
      DO J=1,16
        IF(EB(IZMAX,J).GT.35.0D0) JBM=J
      ENDDO
      WISCUT=MAX(50.0D0,EB(IZMAX,JBM)-0.1D0)
      WRITE(6,'(/1X,'' Inner-shell lowest energy = '',1P,E12.5,
     1  '' eV'')') WISCUT
C
  909 CONTINUE
      NOST=NOS
...
C
  ...
  910 CONTINUE
      NINSH=0
      DO J=1,NOST
        IF(KST(J).LT.17.AND.UIT(J).GT.WISCUT) NINSH=NINSH+1
      ENDDO
      IELIM=0
      IF(NOST.GT.IFCB+1) THEN
  ...
  911   CONTINUE
      ...
      IF(IELIM.GT.0) THEN
        NOST=NOST-IELIM
        IF(NOST.GT.IFCB+NINSH+1) GO TO 910
      ENDIF
...
C        arrays.
      IF(NOST.LT.NO) THEN
        IF(RGROUP.LT.1.5D0) THEN
          RGROUP=RGROUP**2
          IF(NOST.GT.IFCB+NINSH+4) GO TO 910
        ENDIF
        NOSC(M)=NOST
        DO I=1,NOSC(M)
          F(M,I)=FFT(I)
      ...
      ELSE
        RGROUP=RGROUP**2
        IF(RGROUP.GT.2.0D0) THEN
          WISCUT=1.25D0*WISCUT
          WRITE(6,'(1X,'' Inner-shell lowest energy = '',1P,E12.5,
     1      '' eV'')') WISCUT
          GO TO 909
        ENDIF
        GO TO 910
      ENDIF
      WRITE(6,'(/1X,'' E/P inel. grouping factor = '',1P,E12.5)') RGROUP
C
...
C
  ...
  920 CONTINUE
      NINSH=0
      DO J=1,NOST
        IF(KSC(J).LT.17.AND.UIC(J).GT.WISCUT) NINSH=NINSH+1
      ENDDO
      IELIM=0
      IF(NOSTC.GT.IFCB+1) THEN
        DO 921 I=IFCB+1,NOSTC-1
  ...
  921   CONTINUE
      ...
      IF(IELIM.GT.0) THEN
        NOSTC=NOSTC-IELIM
        IF(NOSTC.GT.IFCB+NINSH+1) GO TO 920
      ENDIF
...
C        arrays.
      IF(NOSTC.LT.NOCO) THEN
        IF(RGROUP.LT.1.5D0) THEN
          RGROUP=RGROUP**2
          IF(NOSTC.GT.IFCB+NINSH+4) GO TO 920
        ENDIF
        NOSCCO(M)=NOSTC
        DO I=1,NOSCCO(M)
          FCO(M,I)=FC(I)
...
C
      ...
      IF(IFILE.EQ.1) THEN
        OPEN(7,FILE=MFNAME)
      ELSE
        WRITE(6,*) ' Enter path+name for this file (up to 80 char',
     1    'acters) ...'
        READ(5,'(A80)') PFILE
        OPEN(7,FILE=PFILE)
      ENDIF
C
      WRITE(7,1000)
 1000 FORMAT(1X,
     1  'PENELOPE (v. 2014)  Material data file ...............')
      WRITE(7,1001) NAME
 1001 FORMAT(' Material: ',A62)
      WRITE(7,1002) RHO(M)
 1002 FORMAT(' Mass density =',1P,E15.8,' g/cm**3')
...
C
      WRITE(7,1099)
 1099 FORMAT(1X,
     1  'PENELOPE (v. 2014)  End of material data file ........')
      CLOSE(7)
      RETURN
      END
C  *********************************************************************
...
C                  FUNCTION AVNCOL
C  *********************************************************************
      FUNCTION AVNCOL(E,KPAR,M,ICOL)
C
C  This function computes the mean number of interactions of type ICOL
C  that particles of type KPAR with initial energy E undergo along their
C  complete tracks in material M. If ICOL does not correspond to a hard
C  interaction type, or E is outside the allowed range, the result is
C  set equal to 0.0D0. For photons (KPAR=2) the result is the average
C  number of interactions in a mean free path.
C
      USE PENELOPE_mod
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER*4 (I-N)
...
C
        IF(ICOL.EQ.2) THEN
          DO KE=1,NEGP
            AUX(KE)=EXP(SEHEL(M,KE))/(CSTPE(M,KE)+RSTPE(M,KE))
          ENDDO
          AVNCOL=RMOMX(ET,AUX,EL,E,NEGP,0)
        ELSE IF(ICOL.EQ.3) THEN
          DO KE=1,NEGP
            AUX(KE)=(EXP(SEHIN(M,KE))+EXP(SEISI(M,KE)))
     1        /(CSTPE(M,KE)+RSTPE(M,KE))
          ENDDO
          AVNCOL=RMOMX(ET,AUX,EL,E,NEGP,0)
        ELSE IF(ICOL.EQ.4) THEN
          DO KE=1,NEGP
            AUX(KE)=EXP(SEHBR(M,KE))/(CSTPE(M,KE)+RSTPE(M,KE))
          ENDDO
          AVNCOL=RMOMX(ET,AUX,EL,E,NEGP,0)
        ELSE IF(ICOL.EQ.5) THEN
          DO KE=1,NEGP
            AUX(KE)=EXP(SEISI(M,KE))/(CSTPE(M,KE)+RSTPE(M,KE))
          ENDDO
          AVNCOL=RMOMX(ET,AUX,EL,E,NEGP,0)
        ELSE IF(ICOL.EQ.8) THEN
          DO KE=1,NEGP
            AUX(KE)=EXP(SEAUX(M,KE))/(CSTPE(M,KE)+RSTPE(M,KE))
          ENDDO
          AVNCOL=RMOMX(ET,AUX,EL,E,NEGP,0)
        ELSE IF(ICOL.EQ.0) THEN
          DO KE=1,NEGP
            AUX(KE)=EXP(SETOT(M,KE))/(CSTPE(M,KE)+RSTPE(M,KE))
          ENDDO
          AVNCOL=RMOMX(ET,AUX,EL,E,NEGP,0)
        ENDIF
...
C
    ...
    1   IT=(II+IU)/2
        ...
        ENDIF
        IF(IU-II.GT.1) GO TO 1
        HMF(1)=EXP(XSRA(M,II)+(XSRA(M,II+1)-XSRA(M,II))*(XEL-ERA(II))
     1      /(ERA(II+1)-ERA(II)))
        HMF(2)=EXP(SGCO(M,KE)+(SGCO(M,KE+1)-SGCO(M,KE))*XEK)
        CALL GPHaT(EE,XS,M)
        HMF(3)=XS*VMOL(M)
        IF(E.GT.1.022D6) THEN
          HMF(4)=EXP(SGPP(M,KE)+(SGPP(M,KE+1)-SGPP(M,KE))*XEK)
        ELSE
          HMF(4)=0.0D0
        ENDIF
        AVNCOL=HMF(ICOL)/MAX(HMF(1)+HMF(2)+HMF(3)+HMF(4),1.0D-35)
...
C
        IF(ICOL.EQ.2) THEN
          DO KE=1,NEGP
            AUX(KE)=EXP(SPHEL(M,KE))/(CSTPP(M,KE)+RSTPP(M,KE))
          ENDDO
          AVNCOL=RMOMX(ET,AUX,EL,E,NEGP,0)
        ELSE IF(ICOL.EQ.3) THEN
          DO KE=1,NEGP
            AUX(KE)=(EXP(SPHIN(M,KE))+EXP(SPISI(M,KE)))
     1        /(CSTPP(M,KE)+RSTPP(M,KE))
          ENDDO
          AVNCOL=RMOMX(ET,AUX,EL,E,NEGP,0)
        ELSE IF(ICOL.EQ.4) THEN
          DO KE=1,NEGP
            AUX(KE)=EXP(SPHBR(M,KE))/(CSTPP(M,KE)+RSTPP(M,KE))
          ENDDO
          AVNCOL=RMOMX(ET,AUX,EL,E,NEGP,0)
        ELSE IF(ICOL.EQ.5) THEN
          DO KE=1,NEGP
            AUX(KE)=EXP(SPISI(M,KE))/(CSTPP(M,KE)+RSTPP(M,KE))
          ENDDO
          AVNCOL=RMOMX(ET,AUX,EL,E,NEGP,0)
        ELSE IF(ICOL.EQ.6) THEN
          DO KE=1,NEGP
            AUX(KE)=EXP(SPAN(M,KE))/(CSTPP(M,KE)+RSTPP(M,KE))
          ENDDO
          AVNCOL=RMOMX(ET,AUX,EL,E,NEGP,0)
        ELSE IF(ICOL.EQ.8) THEN
          DO KE=1,NEGP
            AUX(KE)=EXP(SPAUX(M,KE))/(CSTPP(M,KE)+RSTPP(M,KE))
          ENDDO
          AVNCOL=RMOMX(ET,AUX,EL,E,NEGP,0)
        ELSE IF(ICOL.EQ.0) THEN
          DO KE=1,NEGP
            AUX(KE)=EXP(SPTOT(M,KE))/(CSTPP(M,KE)+RSTPP(M,KE))
          ENDDO
          AVNCOL=RMOMX(ET,AUX,EL,E,NEGP,0)
        ENDIF
      ENDIF
...
C  or E is outside the allowed range, the result is set equal to 1.0D35.
C
      USE PENELOPE_mod
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER*4 (I-N)
...
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER*4 (I-N)
C
      IF(XS0.LT.0.0D0) STOP 'EELa0. Negative total cross section.'
 ...
 1001   FORMAT(/3X,'*** The arguments in subroutine EELa0 are inconsi',
     1    'stent.'/7X,'XS0 = ',1P,E14.7,', XS1 = ',E14.7)
        STOP 'EELa0. Inconsistent arguments.'
      ENDIF
C
...
C
      DO IEL=1,NELEM(M)
        ...
        READ(3,*) IZZZ
        IF(IZZZ.NE.IZZ) STOP 'EELaW. Wrong file.'
        DO I=1,NEGP
...
C                       SUBROUTINE EINaT
C  *********************************************************************
      SUBROUTINE EINaT(E,WCCM,XH0,XH1,XH2,XS0,XS1,XS2,XT1,XT2,DELTA,M)
C
...
C  less than the cutoff energy WCCM.
C
C  Sternheimer-Liljequist GOS model.
C
C  Output arguments:
...
C
      DO K=1,NOSC(M)
        UK=UI(M,K)
        WK=WRI(M,K)
        CALL EINaT1(E,UK,WK,DELTA,WCCM,H0,H1,H2,S0,S1,S2,R0,R1,R2)
c		CALL EINaT12(E,UK,WK,DELTA,WCCM,H0,H1,H2,S0,S1,S2,R0,R1,R2)
        SEH0(K)=F(M,K)*H0
        SEH1(K)=F(M,K)*H1
        SEH2(K)=F(M,K)*H2
        SES0(K)=F(M,K)*S0
        ...
        SES2(K)=F(M,K)*S2
        SET0(K)=F(M,K)*R0
        SET1(K)=F(M,K)*2.0D0*R1
        SET2(K)=F(M,K)*6.0D0*(R1-R2)
        XH0=XH0+SEH0(K)
        XH1=XH1+SEH1(K)
        XH2=XH2+SEH2(K)
        XS0=XS0+SES0(K)
...
C                       SUBROUTINE EINaT1
C  *********************************************************************
      SUBROUTINE EINaT1(E,UK,WK,DELTA,WCCM,H0,H1,H2,S0,S1,S2,R0,R1,R2)
C
...
C  and smaller than, the cutoff energy loss WCCM.
C
C  Sternheimer-Liljequist oscillator model.
C
C  Input arguments:
C    E ..... kinetic energy (eV).
C    UK .... ionisation energy (eV).
C    WK .... resonance energy (eV).
C    DELTA ... Fermi's density effect correction.
C    WCCM ... cutoff energy loss (eV).
C
C  Output arguments:
...
C  ****  Fundamental constants and related quantities.
      PARAMETER (REV=5.10998928D5)  ! Electron rest energy (eV)
      PARAMETER (ELRAD=2.8179403267D-13)  ! Class. electron radius (cm)
...
C
      ...
      IF(WCCM.LT.WTHR) THEN  ! No soft interactions.
        WL=WTHR
        WU=WCMAX
...
C  ****  Soft close transport moments of orders 0-2.
        ...
        MOM=0
c        R0=R0+SUMGA(EINaDS,RMU2,RMU3,1.0D-7)
c		write (*,*) 'XL: ', RMU2, ' XU: ', RMU3
c		NNN = 1
c		TOL = 1.0D-7
c		R0=R0+SUMGA2(NNN,RMU2,RMU3,TOL)
		R0=R0+SUMGA(EINaDS,RMU2,RMU3,1.0D-7)
        MOM=1
c       R1=R1+SUMGA2(NNN,RMU2,RMU3,TOL)
		R1=R1+SUMGA(EINaDS,RMU2,RMU3,1.0D-7)
        MOM=2
        R2=R2+SUMGA(EINaDS,RMU2,RMU3,1.0D-7)
C		write(*,*) 'PENELOPE RMU2: ', RMU2, ' RMU3: ', RMU3
C		stop
c		R2=R2+SUMGA2(NNN,RMU2,RMU3,TOL)
...
c	  write(*,*) 'RMU: ', RMU	
c	  EINaDS = EINADS2(RMU);
c	  write(*,*) 'EINAds: ', EINaDS
c	  RETURN
      AUX=2.0D0*RMU*(1.0D0-RMU)
      DENOM=EI*AUX+REV
      W=CPS*AUX/DENOM
      DWDMU=CPS*REV*(2.0D0-4.0D0*RMU)/DENOM**2
...
C                       SUBROUTINE PINaT
C  *********************************************************************
      SUBROUTINE PINaT(E,WCCM,XH0,XH1,XH2,XS0,XS1,XS2,XT1,XT2,DELTA,M)
C
...
C  ****  E/P inelastic collisions.
     ...
     1  WRI(MAXMAT,NO),KZ(MAXMAT,NO),KS(MAXMAT,NO),NOSC(MAXMAT)
C  ****  Partial cross sections of individual shells/oscillators.
      COMMON/CPIN00/SEH0(NO),SEH1(NO),SEH2(NO),SES0(NO),SES1(NO),
     1              SES2(NO),SET0(NO),SET1(NO),SET2(NO)
C
C  ****  Constants.
C
...
C
      DO K=1,NOSC(M)
        UK=UI(M,K)
        WK=WRI(M,K)
c		write(*,*) 'PINAT PENELOPE'
        CALL PINaT1(E,UK,WK,DELTA,WCCM,H0,H1,H2,S0,S1,S2,R0,R1,R2)
        SEH0(K)=F(M,K)*H0
        SEH1(K)=F(M,K)*H1
        SEH2(K)=F(M,K)*H2
        SES0(K)=F(M,K)*S0
...
C                       SUBROUTINE PINaT1
C  *********************************************************************
      SUBROUTINE PINaT1(E,UK,WK,DELTA,WCCM,H0,H1,H2,S0,S1,S2,R0,R1,R2)
C
...
C                       SUBROUTINE ESIa
C  *********************************************************************
      SUBROUTINE ESIa(E,DELTA,DE,EP,CDT,ES,CDTS,M,IZZ,ISH)
C
C  Random sampling of inner-shell ionisation by electron impact.
C
C  Sternheimer-Liljequist GOS model.
C
C  Input arguments:
...
C                       SUBROUTINE ESIb
C  *********************************************************************
      SUBROUTINE ESIb(M,IZZ,ISH)
C
C  Random sampling of the active sell in inner-shell ionisation by
C  electron impact.
C
C  Input arguments:
C    M ....... material where electrons propagate.
C  Output arguments:
C    IZZ ..... atomic number of the element that has been ionized.
C    ISH ..... atomic electron shell that has been ionized.
C
      USE PENELOPE_mod
C
...
C  ****  Electron impact ionisation cross section tables.
      ...
      COMMON/CESI0/XESI(NRP,16),IESIF(99),IESIL(99),NSESI(99),NCURE
      PARAMETER (NDIN=850)
      DIMENSION E(NDIN),XESIR(NDIN,16),X(NDIN),Y(NDIN)
C
      CS5(1) ='CS-K '
      CS5(2) ='CS-L1'
      CS5(3) ='CS-L2'
...
C
 ...
 1001   FORMAT(46X,I3,11X,I3,10X,I4)
 ...
 2001   FORMAT(/1X,'*** Electron impact ionisation cross sections, ',
     1    ' IZ =',I3,',  NSHELL =',I3,',  NDATA =',I4)
        IF(IZZ.NE.IZ(M,IEL))
     1    STOP 'ESIaR. Corrupt material data file.'
        IF(NDATA.GT.NDIN) STOP 'ESIaR. Too many data points.'
        IF(NSHR.GT.16) STOP 'ESIaR. Too many shells.'
        DO IE=1,NDATA
        ...
        IF(INFO.GE.2) THEN
          ...
          DO IE=1,NDATA
            ...
            DO IS=1,NSHR
              TCS=TCS+XESIR(IE,IS)
            ENDDO
            WRITE(IWR,'(1P,20E12.5)') E(IE),(XESIR(IE,IS),IS=1,NSHR),TCS
          ENDDO
        ENDIF
C
        NSESI(IZZ)=NSHR
     ...
     1        NCURE+NEGP
            STOP 'ESIaR. Insufficient memory storage.'
          ENDIF
          DO IS=1,NSHR
            N=0
            DO I=1,NDATA
...
C
C  Data are read from the files 'pdesiZZ.p14'.
C
      USE PENELOPE_mod
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER*4 (I-N)
...
C
      DO IEL=1,NELEM(M)
        ...
        LDIG1=LDIG(NLD1+1)
        LDIG2=LDIG(NLD2+1)
        FILEN='pdesi'//LDIG2//LDIG1//'.p14'
        OPEN(3,FILE='./pdfiles/'//FILEN)
        READ(3,'(16X,I2,6X,I2)') IZZZ,NSHR
        IF(IZZZ.NE.IZZ) STOP 'ESIaW. Corrupt data file.'
        IF(NSHR.GT.16) STOP 'ESIaW. Too many shells.'
        READ(3,'(A12)') LDUMMY
        READ(3,'(A12)') LDUMMY
        DO IE=1,NES
...
C                       SUBROUTINE PSIa
C  *********************************************************************
      SUBROUTINE PSIa(E,DELTA,DE,EP,CDT,ES,CDTS,M,IZZ,ISH)
C
C  Random sampling of inner-shell ionisation by positron impact.
C
C  Sternheimer-Liljequist GOS model.
C
C  Input arguments:
...
C  ****  Positron impact ionisation cross section tables.
      ...
      COMMON/CPSI0/XPSI(NRP,16),IPSIF(99),IPSIL(99),NSPSI(99),NCURP
      PARAMETER (NDIN=800)
      DIMENSION E(NDIN),XPSIR(NDIN,16),X(NDIN),Y(NDIN)
C
      CS5(1) ='CS-K '
      CS5(2) ='CS-L1'
      CS5(3) ='CS-L2'
...
C
 ...
 2001   FORMAT(/1X,'*** Positron impact ionisation cross sections, ',
     1    ' IZ =',I3,',  NSHELL =',I3,',  NDATA =',I4)
        IF(IZZ.NE.IZ(M,IEL)) STOP 'PSIaR. Corrupt material data file.'
        IF(NDATA.GT.NDIN) STOP 'PSIaR. Too many data points.'
        IF(NSHR.GT.16) STOP 'PSIaR. Too many shells.'
        DO IE=1,NDATA
        ...
        IF(INFO.GE.2) THEN
          ...
          DO IE=1,NDATA
            ...
            DO IS=1,NSHR
              TCS=TCS+XPSIR(IE,IS)
            ENDDO
            WRITE(IWR,'(1P,20E12.5)') E(IE),(XPSIR(IE,IS),IS=1,NSHR),TCS
          ENDDO
        ENDIF
C
        NSPSI(IZZ)=NSHR
     ...
     1        NCURP+NEGP
            ...
            WRITE(26,*) 'Increase the value of the parameter NRP to',
     1        NCURP+NEGP
            STOP 'PSIaR. Insufficient memory storage.'
          ENDIF
          DO IS=1,NSHR
            N=0
            DO I=1,NDATA
...
C  writes them on the material data file.
C
C  Data are read from the files 'pdpsiZZ.p14'.
C
      USE PENELOPE_mod
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER*4 (I-N)
...
C
      DO IEL=1,NELEM(M)
        ...
        LDIG1=LDIG(NLD1+1)
        LDIG2=LDIG(NLD2+1)
        FILEN='pdpsi'//LDIG2//LDIG1//'.p14'
        OPEN(3,FILE='./pdfiles/'//FILEN)
        READ(3,'(16X,I2,6X,I2)') IZZZ,NSHR
        IF(IZZZ.NE.IZZ) STOP 'PSIaW. Corrupt data file.'
        IF(NSHR.GT.16) STOP 'PSIaW. Too many shells.'
        READ(3,'(A12)') LDUMMY
        READ(3,'(A12)') LDUMMY
        DO IE=1,NES
...
C                       SUBROUTINE EBRaR
C  *********************************************************************
      SUBROUTINE EBRaR(WCRM,M,IRD,IWR,INFO)
C
...
C                       SUBROUTINE EBRaT
C  *********************************************************************
      SUBROUTINE EBRaT(E,WCRM,XH0,XH1,XH2,XS1,XS2,M)
C
C  Integrated cross sections for bremss emission by electrons of energy
C  E in material M, restricted to energy losses larger than and less
C  than the cutoff energy WCRM.
C
C  Output arguments:
...
C
      ...
      DO IW=1,NBW
        X(IW)=WB(IW)
        Y(IW)=P0(M,KE,IW)
c		write(*,*) 'PENELOPE KE: ', KE, ' XE: ', XE
c		write(*,*) 'PENELOPE P0: ', P0(M,KE,IW)
      ENDDO
...
c	  write(*,*) WCRE,XS1A,XS1B,XS2A,XS2B
C
...
C                       SUBROUTINE PBRaT
C  *********************************************************************
      SUBROUTINE PBRaT(E,WCRM,XH0,XH1,XH2,XS1,XS2,M)
C
...
C  interpolation on a table of PDF values.
C  The independent variable X is assumed to take only positive values.
C
C    X ..... array of values of the variable (in increasing order).
...
C
     ...
     1       (ABS(RKR-RK(IK)).LT.1.0D-6)) THEN
          ...
          ELSE
            ...
            STOP 'BRAR. Corrupt data file (pdbrang.p08).'
          ENDIF
        ENDDO
      ENDDO
C
...
C
     ...
     2          (ABS(RKR-RK(IK)).LT.1.0D-6)) THEN
             ...
             ELSE
               WRITE(26,*) 'Corrupt data file (pdbrang.p08).'
               STOP 'BRAW. Corrupt data file (pdbrang.p08).'
             ENDIF
          ENDDO
        ENDDO
      ENDDO
...
C                       SUBROUTINE PANaR
C  *********************************************************************
      SUBROUTINE PANaR(ECUT)
C
C  Simulation of annihilation of positrons at rest. Annihilation quanta
C  are stored in the secondary stack only when ECUT is less than REV.
C
      USE PENELOPE_mod
      USE TRACK_mod
C
...
C                       SUBROUTINE PANa
C  *********************************************************************
      SUBROUTINE PANa(E,E1,CDT1,E2,CDT2,M)
C
C  Simulation of positron annihilation (either at rest or in flight) in
C  material M. Ei and CDTi are the energies and polar direction cosines
C  of the two annihilation photons.
C
      USE PENELOPE_mod
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER*4 (I-N)
...
C
 ...
 2003   FORMAT(/3X,'Energy',7X,'CS-Rayl',/4X,'(eV)',8X,'(cm**2)',
     1    /1X,25('-'))
      ...
      ENDIF
C
C  ****  Initialisation of the RITA algorithm for random sampling of the
...
C
        ...
        OPEN(3,FILE='./pdfiles/'//FILEN)
        READ(3,*) IZZZ,NQI
        IF(IZZZ.NE.IZZ) STOP 'GRAaW. Corrupt file.'
        IF(NQI.NE.NQ) STOP 'GRAaW. Corrupt file.'
        DO I=1,NQ
          READ(3,*) Q(I),FF(I)
        ENDDO
        CLOSE(3)
...
C                       SUBROUTINE GCOa
C  *********************************************************************
      SUBROUTINE GCOa(E,DE,EP,CDT,ES,CDTS,M,IZZ,ISH)
C
...
C    IZZ ... atomic number of the atom where scattering has occurred.
C    ISH ... atomic electron shell that has been ionized.
C
      USE PENELOPE_mod
C
...
C
    2 CONTINUE
      TST=S*RAND(5.0D0)
C  ****  Binary search.
      IF(TST.LT.PAC(1)) THEN
        ISHELL=1
      ELSE
        ISHELL=1
        JO=NOSCCO(M)+1
    3   I=(ISHELL+JO)/2
        IF(TST.GT.PAC(I)) THEN
          ISHELL=I
        ELSE
          JO=I
        ENDIF
        IF(JO-ISHELL.GT.1) GO TO 3
        ISHELL=ISHELL+1
      ENDIF
C
...
C
      TST=RAND(12.0D0)
C  ****  Binary search.
      IF(TST.LT.PTRSH(M,1)) THEN
        ISHELL=1
      ELSE
        ISHELL=1
        JO=NOSCCO(M)+1
    5   I=(ISHELL+JO)/2
        IF(TST.GT.PTRSH(M,I)) THEN
          ISHELL=I
        ELSE
          JO=I
        ENDIF
        IF(JO-ISHELL.GT.1) GO TO 5
        ISHELL=ISHELL+1
      ENDIF
C
...
C    IZZ ... atomic number of the atom where absorption has occurred.
C    ISH ... atomic electron shell that has been ionized.
C
...
C
        ...
        ELSE
          PCSL=XPH(I,1)
        ENDIF
        PTOT=PTOT+STF(MAT,IEL)*EXP(PCSL)
        ACP(IEL)=PTOT
      ENDDO
      IF(PTOT*VMOL(MAT).GT.SGPH(MAT,KE)) THEN
        WRITE(26,*) 'WARNING: SGPH is less than the actual mac.'
      ENDIF
C
C  ****  Sample the active element.
C
      TST=RAND(1.0D0)*SGPH(MAT,KE)/VMOL(MAT)
      DO IEL=1,NELEM(MAT)
...
C                       SUBROUTINE GPHaT
C  *********************************************************************
      SUBROUTINE GPHaT(E,XS,M)
C
C  Delivers the photoelectric cross section XS (in cm**2) for photons of
C  energy E in material M.
C
      USE PENELOPE_mod
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER*4 (I-N)
...
C
      DO IEL=1,NELEM(M)
        READ(IRD,1001) IZZ,NSHR,NDATA
 1001   FORMAT(40X,I3,11X,I3,10X,I4)
        IF(INFO.GE.2) WRITE(IWR,2001) IZZ,NSHR,NDATA
 2001   FORMAT(/1X,'*** Photoelectric cross sections,  IZ =',I3,
     1    ',  NSHELL =',I3,',  NDATA =',I4)
        IF(IZZ.NE.IZ(M,IEL)) STOP 'GPHaR. Corrupt material data file.'
        IF(NDATA.GT.NDIM) STOP 'GPHaR. Too many data points.'
        IF(NSHR.GT.16) STOP 'GPHaR. Too many shells.'
...
C
     ...
     1        NCUR+NDATA
            STOP 'GPHaR. Insufficient memory storage.'
          ENDIF
          DO IE=1,NDATA
            IC=NCUR+IE
            EPH(IC)=LOG(ER(IE))
...
C  Data are read from the files 'pdgphZZ.p12'.
C
      USE PENELOPE_mod
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER*4 (I-N)
...
C
      DO IEL=1,NELEM(M)
        ...
        LDIG1=LDIG(NLD1+1)
        LDIG2=LDIG(NLD2+1)
        FILEN='pdgph'//LDIG2//LDIG1//'.p12'
        OPEN(3,FILE='./pdfiles/'//FILEN)
        READ(3,'(15X,I2,1X,I3,1X,I2)') IZZZ,NGP,NSHR
        IF(NGP.GT.NPHM) STOP 'GPHaW. Too many energies.'
        IF(IZZZ.NE.IZZ) STOP 'GPHaW. Corrupt file.'
        IF(NSHR.GT.16) STOP 'GPHaW. Too many shells.'
        ISH(1)=0
...
C
      TRIPL=TRIP(MAT,KE)+(TRIP(MAT,KE+1)-TRIP(MAT,KE))*XEK
      IZZ=0
      ISH=30
      ...
      TST=RAND(10.0D0)
C  ****  Binary search.
      IF(TST.LT.PTRSH(MAT,1)) THEN
        ISHELL=1
      ELSE
        ISHELL=1
        JO=NOSCCO(MAT)+1
    4   I=(ISHELL+JO)/2
        IF(TST.GT.PTRSH(MAT,I)) THEN
          ISHELL=I
        ELSE
          JO=I
        ENDIF
        IF(JO-ISHELL.GT.1) GO TO 4
        ISHELL=ISHELL+1
      ENDIF
      IZZ=KZCO(MAT,ISHELL)
      ISH=KSCO(MAT,ISHELL)
...
C  ****  Composition data.
     ...
     1  VMOL(MAXMAT),IZ(MAXMAT,30),NELEM(MAXMAT)
C  ****
      PARAMETER (NEGPP=10000)
      DIMENSION EIT(NEGPP),XGP0(NEGPP),XGT0(NEGPP)
C
C  ****  Building the cross section table.
C
      DO I=1,NEGPP
...
C
      DO IEL=1,NELEM(M)
        ...
        OPEN(3,FILE='./pdfiles/'//FILEN)
        READ(3,*) IZZZ
        IF(IZZZ.NE.IZZ) STOP 'GPPaW. Corrupt file.'
        DO I=1,NEGPP
...
C        is not followed.
      IF(EB(IZ,IS).LT.ECUTR(MAT)) RETURN
C
      NV=1
      ISV(1)=IS
      PAGE0=PAGE
...
C  ****  If LAGE=.TRUE., the particle age is recorded.
      IF(LAGE) THEN
        NVIS=1
        IF(NV.GT.1) THEN  ! Multiple vacancies in the active shell?
          DO ISC=1,NV
            IF(ISV(ISC).EQ.ISP) NVIS=NVIS+1
          ENDDO
        ENDIF
        PAGE=PAGE-(ALW(IZ,ISP)/DBLE(NVIS))*LOG(RAND(2.0D0))
      ENDIF
C
...
C
      ...
      IF(IS2K.EQ.0) THEN
        KPARS=2
        IF(IS1K.LT.17) THEN
          IF(EB(IZ,IS1K).GT.ECUTR(MAT)) THEN
            NV=NV+1
            ISV(NV)=IS1K
            PTIM(NV)=PAGE
          ENDIF
      ...
      ELSE
        ...
        ENDIF
        IF(IS2K.LT.17) THEN
          IF(EB(IZ,IS2K).GT.ECUTR(MAT)) THEN
            NV=NV+1
            ISV(NV)=IS2K
            PTIM(NV)=PAGE
          ENDIF
...
C
      IF(ET(KS).GT.EABS(KPARS,MAT)) THEN
...
C
      ENERGY=1.0D35
      TRPROB=1.0D35
      IF(IZ.LT.3.OR.IZ.GT.99.OR.JS0.LT.1.OR.JS0.GT.16) RETURN
C
      N1=IFIRST(IZ,JS0)
      IF(N1.EQ.0) RETURN
      NL=ILAST(IZ,JS0)
...
C
	  ...
	  CALL transfcrelax(P,ET,F,IS0,IS1,IS2,
     1              IFIRST,ILAST,NCUR,KS,MODER)
	  RETURN
...
C
      ...
      CHARACTER*2 LASYMB
      CHARACTER*5 CH5
      PARAMETER (HBAR=6.58211928D-16)  ! Planck's constant (eV*s)
C  ****  Element data.
      COMMON/CADATA/ATW(99),EPX(99),RSCR(99),ETA(99),EB(99,30),
     1  ALW(99,30),CP0(99,30),IFI(99,30),IKS(99,30),NSHT(99),LASYMB(99)
C  ****  Atomic relaxation data.
      PARAMETER (NRX=60000, NTRAN=2500)
      COMMON/CRELAX/P(NRX),ET(NRX),F(NRX),IS0(NRX),IS1(NRX),IS2(NRX),
     1              IFIRST(99,16),ILAST(99,16),NCUR,KS,MODER
      DIMENSION JS0(NTRAN),JS1(NTRAN),JS2(NTRAN),PR(NTRAN),ER(NTRAN),
     1          WW(NTRAN),FF(NTRAN),KK(NTRAN),IORD(NTRAN),ISR(NTRAN),
     2          IQQ(30),EE(30),ALWR(30),CP0P(30)
C
...
C
      IF(INFO.GE.2) WRITE(IWR,2002)
 2002 FORMAT(/2X,'i',2X,' Shell    f',4X,'Ui (eV)',4X,'Gamma(1/eV)',
     1  2X,'lifetime (s)',4X,'Ji(0)',/1X,68('-'))
      DO IS=1,NSHR
        READ(IRD,5002) ISR(IS),CH5,IQQ(IS),EE(IS),ALWR(IS),CP0P(IS)
        IF(ALWR(IS).GT.0.0D0) THEN
          ALTIME=HBAR/ALWR(IS)
        ELSE
          ALTIME=0.0D0
        ENDIF
 5002   FORMAT(1X,I3,1X,A5,1X,I1,3E13.5)
        IF(INFO.GE.2) WRITE(IWR,2003) ISR(IS),CH5,LSHELL(ISR(IS)),
     1    IQQ(IS),EE(IS),ALWR(IS),ALTIME,CP0P(IS)
 2003   FORMAT(1X,I3,1X,A5,1X,A2,2X,I1,1P,4E13.5)
      ENDDO
C
      IF(NT.GT.0) THEN
...
C
      ...
      DO IS=1,NSHR
        ...
        EB(IZ,ISR(IS))=EE(IS)
        IF(ALWR(IS).GT.0.0D0) THEN
          ALW(IZ,ISR(IS))=HBAR/ALWR(IS)
        ELSE
          ALW(IZ,ISR(IS))=0.0D0
        ENDIF
        CP0(IZ,ISR(IS))=CP0P(IS)
      ENDDO
      IF(NT.EQ.0) THEN
        DO IS=1,16
          IFIRST(IZ,IS)=NCUR+1
...
C
      ...
      DO IS=1,16
        ...
        ENDDO
      ENDDO
      IF(TST.GT.1.0D-12) THEN
        WRITE(6,'(A,1P,E13.6)') 'TST =',TST
        STOP 'RELAXR. Rounding error is too large.'
      ENDIF
      RETURN
      END
C  *********************************************************************
...
C
      ...
      DO I=1,150000
        ...
        IF(IZR.EQ.IZ) THEN
          NT=NT+1
          IF(NT.GT.NM) STOP 'RELAXW. NM needs to be increased.'
          IS0(NT)=IS0R
          IS1(NT)=IS1R
          IS2(NT)=IS2R
          P(NT)=PR
...
C
     ...
     1        ALW(IZ,KS),CP0(IZ,KS)
 2002       FORMAT(1X,I3,1X,A5,1X,I1,1P,3E13.5)
            EE(KS)=EB(IZ,KS)
          ENDIF
        ENDIF
      ENDDO
...
C  ****  Consistency test.
        ...
        IF(TSTE.GT.1.0D-2) THEN
          ...
          STOP 'EELdW. Electron cross section data are corrupt.'
        ENDIF
      ENDDO
C
C  ****  Positrons.
...
C  ****  Consistency test.
        ...
        IF(TSTE.GT.1.0D-2) THEN
          ...
          STOP 'EELdW. Positron cross section data are corrupt.'
        ENDIF
      ENDDO
      RETURN
      END
 ...
 2001 FORMAT(/1X,'*** Electron elastic differential cross sections')
     ...
     1    IELEC,ETS(IE),ECS(IE),ETCS1(IE),ETCS2(IE)
        IF(IELEC.NE.-1.OR.ABS(ETSIE-ETS(IE)).GT.0.1D0) THEN
          ...
          STOP 'EELdR. Error reading electron elastic DCS data.'
        ENDIF
...
C  ****  Consistency test.
        ...
        IF(TSTE.GT.1.0D-4) THEN
          WRITE(IWR,'('' E='',1P,E12.5)') ETS(IE)
          WRITE(IWR,*) ' Electron cross section data are corrupt.'
          STOP 'EELdR. Electron cross section data are corrupt.'
        ENDIF
      ENDDO
C
 ...
 2002 FORMAT(/1X,'*** Positron elastic differential cross sections')
     ...
     1    IELEC,ETS(IE),PCS(IE),PTCS1(IE),PTCS2(IE)
        IF(IELEC.NE.+1.OR.ABS(ETSIE-ETS(IE)).GT.0.1D0) THEN
          ...
          STOP 'EELdR. Error reading positron elastic DCS data.'
        ENDIF
...
C  ****  Consistency test.
        ...
        IF(TSTE.GT.1.0D-4) THEN
          WRITE(IWR,'('' E='',1P,E12.5)') ETS(IE)
          WRITE(IWR,*) ' Positron cross section data are corrupt.'
          STOP 'EELdR. Positron cross section data are corrupt.'
        ENDIF
      ENDDO
C
      NPP=NP
...
C    STF (1:NELEM) ... stoichiometric indexes.
...
C
     ...
     1      STOP 'ELINIT. Corrupt data file.'
...
C
      IF(E.LT.49.999D0.OR.E.GT.1.0001D8) THEN
        ...
        STOP 'DCSEL. Energy out of range.'
      ENDIF
C
      EL=LOG(E)
...
CXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
C                                                                      C
C  NUMERICAL TOOLS     (Francesc Salvat. Barcelona. February, 2012.)   C
C                                                                      C
CXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER*4 (I-N)
      PARAMETER (EPS=1.0D-10,NP=12000,NP2=NP+NP)
      DIMENSION X1(NP),Y1(NP),X2(NP),Y2(NP),XM(NP),YM(NP)
      DIMENSION X(NP2),Y12(NP2)
C
      IF(N1.GT.NP.OR.N2.GT.NP) THEN
        WRITE(6,'(A,I7)') 'NP =',MAX(N1,N2)
        STOP 'MERGE2. Increase the value of the parameter NP.'
      ENDIF
C
      CALL SORT2(X1,Y1,N1)
      CALL SORT2(X2,Y2,N2)
...
C
      IF(N.GT.NP) THEN
        WRITE(6,'(A,I7)') 'NP =',N
        STOP 'SORT2. Increase the value of the parameter NP.'
      ENDIF
C
      IF(N.EQ.1) RETURN
      DO I=1,N
...
C  Output:
C     A(I),B(I),C(I),D(I) (I=1:N) ... spline coefficients.
C
...
C  ****  Auxiliary arrays H(=A) and DELTA(=D).
      DO I=1,N1
        A(I)=X(I+1)-X(I)
        IF(A(I).LT.1.0D-12*MAX(ABS(X(I)),ABS(X(I+1)))) THEN
          WRITE(6,11)
   11     FORMAT(1X,'*** Error in SPLINE: X values not in',
     1      'increasing order.')
...
C  value XC using binary search.
C
C  Input:
C     X(I) (I=1:N) ... grid points (the X values must be in increasing
C                      order).
...
C
C                                        Francesc Salvat. 2 April, 2012.
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER*4 (I-N)
      PARAMETER (NP=10, NP2=2*NP, NP4=4*NP, NOIT=128, NOIT5=NOIT/5,
     1  NCALLT=100000)
...
C           = 3, subintervals are too narrow.
      COMMON/CSUMGA/IERGA,NCALL  ! Error code, no. of function calls.
      COMMON/CSGAWR/ISGAW  ! Controls the printing of warning messages.
	  
c	  NNN = 1
c	  CALL SUMGA2(NNN, XL, XU, TOL)
c	  RETURN
...
C  ****  Global and partial tolerances.
      ...
      TOL3=1.0D-13  ! Round-off protection.
      SUMGA=0.0D0
      IERGA=0
...
C
        X1=XR(I)
        IF(AHH.LT.ABS(X1)*TOL3) IERGA=3  ! The interval is too narrow.
        SP=W(1)*(FCT(X1+XM(1)*HH)+FCT(X1+XP(1)*HH))
        DO J=2,NP
          SP=SP+W(J)*(FCT(X1+XM(J)*HH)+FCT(X1+XP(J)*HH))
        ENDDO
...
C
        X2=X1+H
        IF(AHH.LT.ABS(X2)*TOL3) IERGA=3  ! The interval is too narrow.
...
C
        ...
        IF(ABS(S12-SI).LT.MAX(TOL2*ABS(S12),1.0D-35)) THEN
...
C
      IF(IERGA.EQ.3) THEN  ! Intervals are too narrow.
        IF(NOI.LT.NOIT5) THEN
...
C
      IF(IU.EQ.IL) THEN
        ...
        IF(ABS(DENOM).GT.ZERO) THEN
          Y1=EXP(YIL+(YFL-YIL)*(LOG(X1)-XIL)/DENOM)*X1**MOM
          Y2=EXP(YIL+(YFL-YIL)*(LOG(X2)-XIL)/DENOM)*X2**MOM
        ELSE
          Y1=EXP(YIL)*X1**MOM
          Y2=EXP(YIL)*X2**MOM
        ENDIF
...
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z), INTEGER*4 (I-N)
      PARAMETER (NR=128)
      COMMON/CRNDG3/X(NR),A(NR),B(NR),F(NR),KA(NR),NPM1
      EXTERNAL RAND
...
C
      ...
      call transfcrndg3(X, A, B, F, KA, NPM1)
	  RETURN
      N=NR
      NU=N/4
      XMIN=-3.0D0
...
C	  CALL RITA02(1,XMIN,XMAX,N,NU,ERRM,0)
	  WRITE(*,'(''ERRM:'',D20.8)') ERRM
	  
      IF(N.NE.NR) STOP 'RNDG30: initialisation error (1).'
...
CXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
C                                                                      X
C    TRANSPORT ROUTINES (Francesc Salvat. Barcelona. August, 2013.)    X
C                                                                      X
CXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
...
C  It must be called when a new (primary or secondary) particle track is
C  is started, and when it crosses an interface.
C
      USE PENELOPE_mod
      USE TRACK_mod
C
...
C
      ...
      ELSE IF(KPAR.EQ.3) THEN
        GO TO 3000
      ELSE
        STOP 'KNOCK: Incorrect particle type.'
      ENDIF
C
C  ************  Electrons (KPAR=1).
C
...
C  ****  New energy.
      ...
      RETURN
C
C  ****  Ionisation of an inner shell (ICOL=5).
C
 1500 ICOL=5
      DELTA=DEL(MAT,KE)+(DEL(MAT,KE+1)-DEL(MAT,KE))*XEK
...
C  ****  Positron.
      IF(EP.GT.EABS(3,MAT)) THEN
        ...
        ILBA(4)=0
        ILBA(5)=ILB(5)
        CALL STORES(EP,X,Y,Z,US,VS,WS,WGHT,3,ILBA,0)
...
C  ****  Angular deflection.
C
      IF(T1P(MAT,KE+1).GT.-78.3D0) THEN
...
C
      UV=U*U+V*V
      UVW=UV+W*W
      IF(ABS(UVW-1.0D0).GT.1.0D-13) THEN
        FNORM=1.0D0/SQRT(UVW)
        U=FNORM*U
        V=FNORM*V
        W=FNORM*W
        UV=U*U+V*V
...
C
      IF(1.0D0-ABS(CDT).GT.1.0D-8) THEN
        SDT=SQRT(1.0D0-CDT*CDT)
      ELSE
        SDT=SQRT(2.0D0*(1.0D0-ABS(CDT)))
      ENDIF
C
      IF(SDT.LT.1.0D-13) THEN
        IF(CDT.LT.0.0D0) THEN
          U=-U
          V=-V
          W=-W
...
C
      ...
      PHA=CDT21+CONS
      PHB=1.0D0-CDT2
      SP0MAX=PHA+PHB*DSQRT(SP1*SP1+SP3*SP3+1.0D-35)
    1 CONTINUE
      DF=RAND(1.0D0)*TWOPI
      SDF=SIN(DF)
      CDF=COS(DF)
...
C     KPARI ........ type of particle (1: electron, 2: photon,
C                    3: positron).
C     ILBI(5) ...... particle labels.
C     IPOLI ........ polarisation flag.
C
...
C
 ...
 1001     FORMAT(/3X,'*** WARNING: (STORES) not enough storage for ',
     1      'secondaries.',/16X,'EABS(KPAR,MAT) or the parameter NMS ',
...
C  ****  Current IMFPs.
      COMMON/CJUMP0/P(8),ST,DST,DSR,W1,W2,T1,T2
C
      P(2)=EXP(SEHEL(MAT,KE)+(SEHEL(MAT,KE+1)-SEHEL(MAT,KE))*XEK)
      P(3)=EXP(SEHIN(MAT,KE)+(SEHIN(MAT,KE+1)-SEHIN(MAT,KE))*XEK)
      P(4)=EXP(SEHBR(MAT,KE)+(SEHBR(MAT,KE+1)-SEHBR(MAT,KE))*XEK)
      P(5)=EXP(SEISI(MAT,KE)+(SEISI(MAT,KE+1)-SEISI(MAT,KE))*XEK)
      P(8)=0.0D0
      IF(IEND.EQ.1) RETURN
      IF(W1E(MAT,KE+1).GT.-78.3D0) THEN
        W1=EXP(W1E(MAT,KE)+(W1E(MAT,KE+1)-W1E(MAT,KE))*XEK)
        W2=EXP(W2E(MAT,KE)+(W2E(MAT,KE+1)-W2E(MAT,KE))*XEK)
      ELSE
        W1=0.0D0
        W2=0.0D0
      ENDIF
...
C  ****  Current IMFPs.
      COMMON/CJUMP0/P(8),ST,DST,DSR,W1,W2,T1,T2
C
      P(2)=EXP(SPHEL(MAT,KE)+(SPHEL(MAT,KE+1)-SPHEL(MAT,KE))*XEK)
      P(3)=EXP(SPHIN(MAT,KE)+(SPHIN(MAT,KE+1)-SPHIN(MAT,KE))*XEK)
      P(4)=EXP(SPHBR(MAT,KE)+(SPHBR(MAT,KE+1)-SPHBR(MAT,KE))*XEK)
      P(5)=EXP(SPISI(MAT,KE)+(SPISI(MAT,KE+1)-SPISI(MAT,KE))*XEK)
      P(6)=EXP(SPAN(MAT,KE)+(SPAN(MAT,KE+1)-SPAN(MAT,KE))*XEK)
      P(8)=0.0D0
      IF(IEND.EQ.1) RETURN
      IF(W1P(MAT,KE+1).GT.-78.3D0) THEN
        W1=EXP(W1P(MAT,KE)+(W1P(MAT,KE+1)-W1P(MAT,KE))*XEK)
        W2=EXP(W2P(MAT,KE)+(W2P(MAT,KE+1)-W2P(MAT,KE))*XEK)
      ELSE
        W1=0.0D0
        W2=0.0D0
      ENDIF
...
C
      P(1)=SGRA(MAT,KE)
      P(2)=EXP(SGCO(MAT,KE)+(SGCO(MAT,KE+1)-SGCO(MAT,KE))*XEK)
      P(3)=SGPH(MAT,KE)
      IF(E.LT.1.023D6) THEN
        P(4)=0.0D0
      ELSE
        P(4)=EXP(SGPP(MAT,KE)+(SGPP(MAT,KE+1)-SGPP(MAT,KE))*XEK)
      ENDIF
      P(8)=0.0D0
      RETURN
      END
